...
def test_102208(setup_driver):
    """
    102208 验证邮箱为空，无法进行下一步操作
    1. 打开APP，点击首页"2注册"按钮
    2. 进入Sign Up页面
    3. 邮箱不输入任何内容
    4. 勾选隐私政策与用户协议
    ...
    try:
        ...
        try:
            ...
            email_input.clear()
            print(f"✅ {current_step} - 完成，邮箱保持为空")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法找到邮箱输入框 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise
        ...
        print("🎉 测试用例102208执行成功！")
        print('✅ 验证邮箱为空，勾选隐私政策后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102208_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102208",
            case_desc='102208 验证邮箱为空，无法进行下一步操作',
            result=case_result,
