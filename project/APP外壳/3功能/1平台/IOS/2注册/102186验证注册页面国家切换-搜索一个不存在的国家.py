...
def test_102186(setup_driver):
    ...
    try:
        ...
        # 步骤6: 点击搜索框并输入"zhonghuaminguo"
        current_step = "步骤6: 点击搜索框并输入zhonghuaminguo"
        print(f"🔄 {current_step}")
        try:
            ...
            time.sleep(1)
            search_field.clear()
            search_field.send_keys("zhonghuaminguo")
            print(f"✅ {current_step} - 完成，已输入zhonghuaminguo")
        ...
        # 步骤7: 验证无任何国家选择结果
        current_step = "步骤7: 验证无任何国家选择结果"
        print(f"🔄 {current_step}")
        try:
            # 查找所有可能的国家选项元素
            # 常见的国家选项元素类型
            country_option_selectors = [
                '//XCUIElementTypeStaticText[contains(@name, "zhonghuaminguo")]',
                '//XCUIElementTypeCell[contains(@name, "zhonghuaminguo")]',
                '//XCUIElementTypeButton[contains(@name, "zhonghuaminguo")]',
            ]
            
            # 也检查一些常见的国家名称，确保没有误匹配
            common_country_keywords = ["China", "France", "United States", "America", "Germany", "Japan"]
            ...
            # 查找所有可见的国家文本元素，检查是否有误匹配
            try:
                all_text_elements = driver.find_elements(AppiumBy.XPATH, '//XCUIElementTypeStaticText')
                for elem in all_text_elements[:50]:  # 限制检查数量，避免过慢
                    try:
                        if elem.is_displayed():
                            text = elem.get_attribute("name")
                            if text:
                                # 检查是否包含搜索关键词或常见国家名称
                                if "zhonghuaminguo" in text.lower() or any(keyword.lower() in text.lower() for keyword in common_country_keywords):
                                    # 如果文本看起来像国家名称，记录下来
                                    if len(text) > 2 and len(text) < 50:  # 合理的国家名称长度
                                        found_countries.append(text)
                    except:
                        continue
            except:
                pass
            
            print(f"📝 搜索关键词: zhonghuaminguo")
            print(f"📝 找到的元素数量: {len(found_countries)}")
            if found_countries:
                print(f"📝 找到的元素: {found_countries[:10]}")  # 只显示前10个
            
            # 验证没有找到任何国家选择结果
            # 如果找到了包含搜索关键词的元素，但应该是空结果或无匹配
            # 这里我们验证：不应该有可点击的国家选项
            clickable_country_found = False
                        ...
                        for elem in elements[:20]:  # 限制检查数量
                            try:
                                if elem.is_displayed() and elem.is_enabled():
                                    elem_text = elem.get_attribute("name") or ""
                                    # 如果元素文本包含搜索关键词，说明可能有匹配结果
                                    if "zhonghuaminguo" in elem_text.lower():
                                        clickable_country_found = True
                                        print(f"⚠️ 发现可点击元素: {elem_text}")
                                        break
                            except:
                                continue
                        if clickable_country_found:
                            break
                    except:
                        continue
            except:
            ...
            # 断言：不应该有可点击的国家选项
            assert not clickable_country_found, \
                f"不应该找到任何国家选择结果，但找到了可点击的国家选项"
            
            print(f"✅ {current_step} - 完成")
            print(f"✅ 验证通过：搜索不存在的国家'zhonghuaminguo'时，无任何国家选择结果")
            time.sleep(2)
            ...
            fail_reason = f"{current_step}失败: 验证过程中发生错误 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        print("🎉 测试用例102186执行成功！")
        print("✅ 注册页面国家切换-搜索一个不存在的国家：无任何国家选择，用例执行成功")
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102186_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102186",
            case_desc="验证注册页面国家切换-搜索一个不存在的国家",
            result=case_result,
