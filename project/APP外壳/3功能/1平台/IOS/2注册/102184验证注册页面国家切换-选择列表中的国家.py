...
def test_102184(setup_driver):
    ...
    try:
        ...
        # 步骤6: 点击搜索框并输入"america"
        current_step = "步骤6: 点击搜索框并输入america"
        print(f"🔄 {current_step}")
        try:
            ...
            time.sleep(1)
            search_field.clear()
            search_field.send_keys("america")
            print(f"✅ {current_step} - 完成，已输入america")
        ...
        # 步骤7: 选择"United States of America"
        current_step = "步骤7: 选择United States of America"
        print(f"🔄 {current_step}")
        try:
            expected_country = "United States of America"

            def get_selected_country_text():
                country_display_selectors = [
                    '//XCUIElementTypeStaticText[@name="United States of America"]',
                    '//XCUIElementTypeStaticText[contains(@name, "United States")]',
                    '//XCUIElementTypeStaticText[contains(@name, "America")]',
                    '//XCUIElementTypeStaticText[@name="American Samoa"]',
                ]
            ...
            try:
                clicked_country = click_country_option_by_visible_text(
                    driver,
                    ["United States of America", "United States", "America"],
                    timeout=10,
                )
            except Exception as text_click_error:
                # 真机上国家项有时肉眼可见，但不会完整暴露到页面树里；
                # 此时退回到“基于搜索框位置的坐标点击”，并在每次点击后立即校验选中的国家。
                clicked_country = None
                last_tap_error = text_click_error
                coordinate_attempt_offsets = [145, 160, 125, 95, 180]

                for idx, offset in enumerate(coordinate_attempt_offsets):
                    try:
                        if idx > 0:
                            # 前一次点错国家后，重新打开国家列表并恢复搜索条件
                        ...
                        search_rect = search_field.rect
                        screen_size = driver.get_window_size()
                        tap_x = int(search_rect["x"] + search_rect["width"] * 0.5)
                        tap_y = int(search_rect["y"] + search_rect["height"] + offset)
                        if tap_y >= screen_size["height"]:
                            continue

                        driver.execute_script("mobile: tap", {"x": tap_x, "y": tap_y})
                        print(
                            f"ℹ️ 文本定位失败，尝试坐标兜底点击国家项: x={tap_x}, y={tap_y}，"
                            f"offset={offset}，原因: {text_click_error}"
                        )
                        time.sleep(2)

                        WebDriverWait(driver, 5).until(
                            EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeImage[@name="login_arrow"]'))
                        )
                        selected_country = get_selected_country_text()
                        print(f"📝 坐标点击后当前国家显示: {selected_country}")

                        if selected_country == expected_country:
                            clicked_country = f"坐标点击({tap_x}, {tap_y})"
                            break
                    except Exception as tap_error:
                        last_tap_error = tap_error
                        continue

                if clicked_country is None:
                    raise Exception(f"{text_click_error}; 坐标兜底点击失败: {last_tap_error}")

            print(f"✅ {current_step} - 完成，已选择国家项: {clicked_country}")
            time.sleep(3)  # 等待返回注册页面并更新国家显示
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法找到或点击United States of America选项 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise
        ...
        print(f"🔄 {current_step}")
        try:
            # 验证注册页面上显示的国家是"United States of America"
            # 查找显示国家名称的元素
            country_display_selectors = [
                '//XCUIElementTypeStaticText[@name="United States of America"]',
                '//XCUIElementTypeStaticText[contains(@name, "United States")]',
        ...
        print("🎉 测试用例102184执行成功！")
        print("✅ 注册页面国家切换功能正常，选择的国家已正确显示")
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102184_failed")
