...
def test_102207(setup_driver):
    """
    102207 验证邮箱数字+字母，无法下一步
    1. 进入 Sign Up 页面
    2. 邮箱框输入数字+字母（如：123456abcd），勾选隐私框
    3. 点击 Next 按钮
    4. 断言页面提示：
       //XCUIElementTypeStaticText[@name="Please sign up using your email address"]
       且仍停留在当前页面（Sign Up 标题仍存在）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤4: 邮箱框输入数字+字母（value="Email"）
        current_step = '步骤4: 邮箱框输入数字+字母（value="Email"）'
        print(f"🔄 {current_step}")
        try:
            email_mixed = "123456abcd"
            email_selectors = [
            ...
            email_input.clear()
            email_input.send_keys(email_mixed)
            print(f"✅ {current_step} - 完成，输入邮箱: {email_mixed}")
            time.sleep(1)
        except Exception as e:
        ...
        try:
            ...
            print(f"ℹ️ {current_step} - 未找到Done按钮或无法点击，跳过: {e}")
            time.sleep(0.5)
        ...
        try:
            ...
            try:
                ...
                # 禁用态点击可能抛异常，但不应直接判失败；下一步用提示文案来判定结果
        ...
        try:
            ...
            error_text = err.get_attribute("name") or ""
        ...
        print("🎉 测试用例102207执行成功！")
        print('✅ 验证邮箱数字+字母，勾选隐私政策后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        write_report(
            ...
            platform="ios",
            case_id="102207",
            case_desc='102207 验证邮箱数字+字母，无法下一步',
            result=case_result,
