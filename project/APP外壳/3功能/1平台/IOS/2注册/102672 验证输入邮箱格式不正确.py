...
def test_102672(setup_driver):
    ...
    try:
        ...
        # 步骤4: 邮箱输入错误格式 adn只s@163.com（包含中文字符）
        current_step = "步骤4: 邮箱输入错误格式"
        print(f"🔄 {current_step}")
        try:
            wrong_email = "adn只s@163.com"
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102672_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102672",
            case_desc='102672 验证错误邮箱，无法进行下一步操作-a@@b.（ab代表n个数字或字母）',
            result=case_result,
