...
def test_102247(setup_driver):
    ...
    try:
        ...
        # 步骤9: 验证用户名输入框存在但不输入任何信息
        current_step = "步骤9: 验证用户名输入框存在但不输入任何信息"
        print(f"🔄 {current_step}")
        try:
            ...
            # 验证输入框为空
            input_value = username_input.get_attribute("value") or ""
            # iOS 真机上：清空后常见会回填占位文案 "Username"
            assert input_value in ["", "Username", None], f"用户名输入框应为空/占位，但实际有内容: {input_value}"
            print(f"✅ {current_step} - 完成，用户名输入框为空")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法找到用户名输入框或验证失败 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        # 步骤10: 点击左上角返回按钮
        current_step = "步骤10: 点击左上角返回按钮"
        print(f"🔄 {current_step}")
        try:
        ...
        # 步骤12: 验证用户名显示为默认的"Username"
        current_step = "步骤12: 验证用户名显示为默认的'Username'"
        print(f"🔄 {current_step}")
        try:
            # 尝试多个可能的选择器
            username_selectors = [
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102247_failed")
