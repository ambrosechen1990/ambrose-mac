...
def test_102206(setup_driver):
    """
    102206 验证注册时输入纯数字时，弹出的错误提示是否正确
    1. 打开APP，点击首页"2注册"按钮
    2. 进入Sign Up页面
    3. 邮箱输入纯数字
    4. 勾选隐私政策与用户协议
    5. 点击Next按钮
    6. 断言停留在当前页面，并出现提示"Please sign up using your email address"
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤4: 邮箱输入纯数字
        current_step = "步骤4: 邮箱输入纯数字"
        print(f"🔄 {current_step}")
        try:
            email_numeric = "1234567890"
            email_input = WebDriverWait(driver, 8).until(
                EC.element_to_be_clickable((AppiumBy.IOS_PREDICATE, 'type == "XCUIElementTypeTextField"'))
            )
            email_input.clear()
            email_input.send_keys(email_numeric)
            print(f"✅ {current_step} - 完成，输入纯数字邮箱: {email_numeric}")
            time.sleep(2)
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                # 真机上 Next 有时可见但不满足 clickable，退回到坐标点击
                rect = next_btn.rect
                tap_x = int(rect["x"] + rect["width"] * 0.5)
                tap_y = int(rect["y"] + rect["height"] * 0.5)
        ...
        print("🎉 测试用例102206执行成功！")
        print('✅ 验证输入纯数字邮箱，勾选隐私政策后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102206_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102206",
            case_desc='102206 验证注册时输入纯数字时，弹出的错误提示是否正确',
            result=case_result,
