...
def test_102204(setup_driver):
    """
    102204 验证勾选“隐私政策、用户协议”后，再填入邮箱，“Next”按钮颜色变深，可以点击
    1. 打开APP，点击首页"2注册"按钮
    2. 进入Sign up页面
    3. 输入邮箱
    4. 勾选隐私政策与用户协议
    ...
    try:
        ...
        print(f"🔄 {current_step}")
        try:
            next_btn_to_click = WebDriverWait(driver, 8).until(
        ...
        # 步骤7: 验证跳转到set password页面
        current_step = "步骤7: 验证跳转到set password页面"
        print(f"🔄 {current_step}")
        try:
            ...
            print(f"✅ {current_step} - 完成")
            print("✅ 验证通过：跳转到set password页面，用例执行成功")
            time.sleep(2)
        except Exception as e:
        ...
        print("🎉 测试用例102204执行成功！")
        print('✅ 验证勾选隐私政策后再填邮箱，Next变深可点击且跳转到set password页面')
        time.sleep(2)
        ...
        write_report(
            ...
            platform="ios",
            case_id="102204",
            case_desc='102204 验证勾选“隐私政策、用户协议”后，再填入邮箱，“Next”按钮颜色变深，可以点击',
            result=case_result,
