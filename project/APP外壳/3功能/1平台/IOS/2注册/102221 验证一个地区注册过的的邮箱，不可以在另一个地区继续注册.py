...
def test_102221(setup_driver):
    """
    102221 验证一个地区注册过的的邮箱，不可以在另一个地区继续注册
    1) 登出确保初始状态；首页点击 Sign Up
    2) 国家切换至 France
    3) 使用已在美国注册过的邮箱 haoc51888@gmail.com
    4) 勾选隐私政策/用户协议
    5) 点击 Next，弹框提示 This email is already registered. Ready to sign in now?
    6) 断言弹框出现（无需跳转）
    """
    driver = setup_driver
    case_result = "success"
    ...
    try:
        ...
        try:
            ...
            assert_on_country_select_page(driver, timeout=10)
            ...
            time.sleep(2)
            click_country_option_by_visible_text(driver, ["France"], timeout=10)
            time.sleep(3)
        ...
        try:
            ...
            assert confirm_btn.is_displayed(), "弹框中未显示 Confirm 按钮"
            assert cancel_btn.is_displayed(), "弹框中未显示 Cancel 按钮"
        ...
        print("🎉 测试用例102221执行成功！")
        print('✅ 已注册邮箱在France再次注册，弹出已注册提示（未继续下一步）')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102221_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102221",
            case_desc='102221 验证一个地区注册过的的邮箱，不可以在另一个地区继续注册',
            result=case_result,
