...
def test_102230(setup_driver):
    """
    102230 验证密码为纯大写字母时，“下一步”按钮不可点击
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码输入 XINGMAI，规则期望：
       • 6-20 characters 灰
       • contains letters 灰
       • contains numbers 红
    ...
    try:
        ...
        # 步骤8: 输入纯大写密码并验证规则颜色
        current_step = "步骤8: 输入纯大写密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102230执行成功！")
        print('✅ 纯大写密码规则提示符合预期，Next不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102230_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102230",
            case_desc='102230 验证密码为纯大写字母时，“下一步”按钮不可点击',
            result=case_result,
