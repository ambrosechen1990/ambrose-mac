...
def test_102226(setup_driver):
    """
    102226 验证密码等于6个字符时，“下一步”按钮可点击
    1) 登出后首页 Sign Up → 进入注册页
    2) 输入邮箱（get_simple_email()），勾选隐私协议，点击Done收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码均输入 Csx150
    5) 点击Done收起键盘，点击密码页 Next 进入用户设置页
    6) 点击 Submit，跳转主页面（通过首页标识元素验证）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        current_step = "步骤10: 用户名页面点击Skip并验证跳转主页面"
        print(f"🔄 {current_step}")
        try:
            # 兼容多种 Skip 元素形态
            skip_selectors = [
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102226_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102226",
            case_desc='102226 验证密码等于6个字符时，“下一步”按钮可点击',
            result=case_result,
