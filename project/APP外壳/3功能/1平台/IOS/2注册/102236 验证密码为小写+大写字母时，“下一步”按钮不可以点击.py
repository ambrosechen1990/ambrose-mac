...
def test_102236(setup_driver):
    """
    102236 验证密码为小写+大写字母时，“下一步”按钮不可以点击
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码输入 XINGmai（大小写混合）
    ...
    try:
        ...
        # 步骤8: 输入大小写混合密码并验证规则颜色
        current_step = "步骤8: 输入大小写混合密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102236执行成功！")
        print('✅ 大小写混合但无数字/特殊字符，Next不可跳转，停留密码页')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102236_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102236",
            case_desc='102236 验证密码为小写+大写字母时，“下一步”按钮不可以点击',
            result=case_result,
