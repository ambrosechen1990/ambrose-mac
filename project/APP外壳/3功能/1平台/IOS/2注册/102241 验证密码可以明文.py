...
def test_102241(setup_driver):
    ...
    4) 密码框输入 Csx150128
    5) 点击显示密码按钮 //XCUIElementTypeButton[@name="login pwd hide"]
    6) 断言密码可明文（值等于输入的密码），测试通过
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                show_btn.click()
                print("✅ 已点击显示密码按钮（login pwd hide）")
                time.sleep(1)
            except Exception as e:
                fail_reason = f"显示密码按钮未找到或不可点击: {e}"
                print(f"❌ {fail_reason}")
                raise
            ...
            value_after = plain_password_input.get_attribute("value") or ""
            field_type = plain_password_input.get_attribute("type") or ""
            print(f"📝 显示后密码框value: '{value_after}', type: '{field_type}'")
            assert value_after == pwd, f"点击显示后应看到明文，当前: '{value_after}'"
            print("✅ 密码已明文显示")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
        ...
        print("🎉 测试用例102241执行成功！")
        print('✅ 密码可切换为明文显示校验通过')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102241_failed")
