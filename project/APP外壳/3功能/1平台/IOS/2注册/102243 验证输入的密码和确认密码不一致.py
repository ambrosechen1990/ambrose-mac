...
def test_102243(setup_driver):
    """
    102243 验证输入的密码和确认密码不一致
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码框输入 Csx150128，确认密码输入 Csx1501（不一致）
    5) Done 收起键盘，点击密码页 Next，期望停留当前页面
    6) 断言页面显示 Password mismatch 提示
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 输入不一致的密码和确认密码
        current_step = "步骤8: 输入不一致的密码和确认密码"
        print(f"🔄 {current_step}")
        try:
            ...
            retype_password_input.send_keys(pwd_mismatch)
            print(f"✅ 已输入确认密码(不一致): {pwd_mismatch}")
            time.sleep(1)
        ...
        # 步骤9: 点击Next并验证停留当前页面且提示密码不一致
        current_step = "步骤9: 点击Next验证仍停留并提示密码不一致"
        print(f"🔄 {current_step}")
        try:
            ...
            # 提示 Password mismatch
            mismatch_tip = WebDriverWait(driver, 6).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Password mismatch"]'))
            )
            assert mismatch_tip.is_displayed(), "未看到Password mismatch提示"
            print("✅ 提示已显示: Password mismatch，页面未跳转")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print("🎉 测试用例102243执行成功！")
        print('✅ 密码与确认密码不一致时，Next不跳转且提示Password mismatch')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102243_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102243",
            case_desc='102243 验证输入的密码和确认密码不一致',
            result=case_result,
