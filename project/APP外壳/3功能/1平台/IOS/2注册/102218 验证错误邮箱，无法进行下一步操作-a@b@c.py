...
def test_102218(setup_driver):
    """
    102218 验证错误邮箱，无法进行下一步操作-a@b@c
    1) 登出确保初始状态；首页 Sign Up 进入注册页
    2) 邮箱输入错误格式 asd@gmail@163.com
    3) 勾选隐私政策/用户协议复选框
    4) 点击 Next
    5) 断言仍在注册页（隐私协议元素在），并提示 Please sign up using your email address
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤4: 邮箱输入错误格式 asd@gmail@163.com
        current_step = "步骤4: 邮箱输入错误格式"
        print(f"🔄 {current_step}")
        try:
            wrong_email = "asd@gmail@163.com"
        ...
        print("🎉 测试用例102218执行成功！")
        print('✅ 验证错误邮箱 asd@gmail@163.com 勾选隐私后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102218_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102218",
            case_desc='102218 验证错误邮箱，无法进行下一步操作-a@b@c',
            result=case_result,
