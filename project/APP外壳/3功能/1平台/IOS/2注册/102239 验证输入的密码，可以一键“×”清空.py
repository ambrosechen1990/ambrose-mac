...
def test_102239(setup_driver):
    """
    102239 验证输入的密码，可以一键“×”清空
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码框输入 Csx150128（确认密码同样输入，以便显示清除按钮）
    5) 点击密码输入框的清除按钮 //XCUIElementTypeButton[@name="login delete"]
    6) 断言密码框内容被清空（值为空或占位值），用例成功
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤10: 验证密码被清空
        current_step = "步骤10: 验证密码被清空"
        print(f"🔄 {current_step}")
        try:
            pwd_input_after = WebDriverWait(driver, 6).until(
                EC.presence_of_element_located((AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSecureTextField[1]'))
            )
            value_after = pwd_input_after.get_attribute("value") or ""
            print(f"📝 清除后密码框内容: '{value_after}'")
            # iOS 清空后可能返回空字符串，也可能返回占位文案（Password）
        ...
        print("🎉 测试用例102239执行成功！")
        print('✅ 验证输入的密码可以一键“×”清空')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102239_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102239",
            case_desc='102239 验证输入的密码，可以一键“×”清空',
            result=case_result,
