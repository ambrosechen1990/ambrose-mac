...
def test_102224(setup_driver):
    """
    102224 验证密码小于6个字符时，“下一步”按钮不可点击
    1) 登出后首页 Sign Up → 进入注册页
    2) 输入邮箱（get_simple_email()），勾选隐私协议
    3) 点击 Next 进入 set password 页面
    4) 密码框输入 Csx15（<6位）
    5) 断言四条规则颜色：
       • 6-20 characters - 红色
       • contains letters - 灰色
       • contains numbers - 灰色
       • Supports special characters:! @ # $ % ^ & * ( ) - _ = + \ | [ ] { } ; : / ? . , ~ > < ` - 灰色
    6) 收起键盘 Done，点击 Next，仍停留在当前页面
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 输入小于6位密码并验证规则颜色
        current_step = "步骤8: 输入小于6位密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
            ...
            password_input.send_keys("Csx15")
            print("✅ 已输入密码: Csx15")
            time.sleep(1.5)
        ...
        print("🎉 测试用例102224执行成功！")
        print('✅ 密码小于6位时规则颜色符合预期，Next按钮不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102224_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102224",
            case_desc='102224 验证密码小于6个字符时，“下一步”按钮不可点击',
            result=case_result,
