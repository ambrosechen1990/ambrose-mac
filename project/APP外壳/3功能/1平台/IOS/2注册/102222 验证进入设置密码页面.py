...
def test_102222(setup_driver):
    """
    102222 验证进入设置密码页面
    1) 登出后首页点击 Sign Up 进入注册页
    2) 输入邮箱（使用 email_utils）
    3) 勾选隐私政策/用户协议复选框
    4) 点击 Next 进入 set password 页面
    5) 不输入密码，验证 Next 为浅色/禁用，点击后仍停留在当前页面
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 不输入密码，验证Next为浅色/禁用并点击后仍停留本页
        current_step = "步骤8: 不输入密码验证Next不可点击，点击后仍停留本页"
        print(f"🔄 {current_step}")
        try:
            next_btn_password = WebDriverWait(driver, 8).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Next"]'))
            )
            is_enabled_pwd = next_btn_password.is_enabled()
            print(f"📝 密码页Next按钮enabled状态: {is_enabled_pwd}")
            if is_enabled_pwd:
                print("ℹ️ Next按钮可能仍返回可点击，将尝试点击后通过停留页面验证禁用行为")
            try:
                next_btn_password.click()
                print("✅ 已尝试点击密码页Next按钮")
            except Exception as click_err:
                print(f"ℹ️ 点击Next按钮未触发（可能禁用）: {click_err}")
            time.sleep(2)
            ...
            print("✅ 点击后仍停留在set password页面，Next按钮视为禁用/无效状态")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print("🎉 测试用例102222执行成功！")
        print('✅ 验证进入设置密码页面：不填密码时Next为浅色/无效，点击后仍停留在当前页面')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102222_failed")
