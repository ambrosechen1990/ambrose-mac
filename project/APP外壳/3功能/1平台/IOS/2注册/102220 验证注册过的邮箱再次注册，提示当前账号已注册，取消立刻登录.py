...
def test_102220(setup_driver):
    """
    102220 验证注册过的邮箱再次注册，提示当前账号已注册，取消立刻登录
    1) 登出后首页点击 Sign Up 进入注册页
    2) 输入已注册邮箱 haoc51888@gmail.com
    3) 勾选隐私政策/用户协议复选框
    4) 点击 Next，弹出提示 This email is already registered. Ready to sign in now?
    5) 点击 Cancel
    6) 断言停留在当前页面，页面有隐私协议元素显示
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 点击Cancel
        current_step = "步骤8: 点击Cancel"
        print(f"🔄 {current_step}")
        try:
            cancel_btn = WebDriverWait(driver, 6).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Cancel"]'))
            )
            cancel_btn.click()
            print("✅ 已点击Cancel")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法点击Cancel按钮 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        # 步骤9: 断言仍停留在注册页面（隐私协议元素存在）
        current_step = "步骤9: 断言仍停留在注册页面"
        print(f"🔄 {current_step}")
        try:
            privacy_element_after = WebDriverWait(driver, 6).until(
            ...
            assert privacy_element_after.is_displayed(), "取消后应仍在注册页面，未找到隐私协议元素"
            print(f"✅ {current_step} - 完成，仍停留在注册页面")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: 取消后未停留在注册页面 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        print("🎉 测试用例102220执行成功！")
        print('✅ 已注册邮箱再次注册，弹框后点击Cancel，仍停留在注册页')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102220_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102220",
            case_desc='102220 验证注册过的邮箱再次注册，提示当前账号已注册，取消立刻登录',
            result=case_result,
