...
def test_102231(setup_driver):
    """
    102231 验证密码为纯字符时，“下一步”按钮不可点击
    1) 登出后首页 Sign Up → 注册页
    ...
    try:
        ...
        # 步骤8: 输入纯特殊字符密码并验证规则颜色
        current_step = "步骤8: 输入纯特殊字符密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102231执行成功！")
        print('✅ 纯字符密码规则提示符合预期，Next不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102231_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102231",
            case_desc='102231 验证密码为纯字符时，“下一步”按钮不可点击',
            result=case_result,
