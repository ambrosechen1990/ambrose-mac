...
def test_102180(setup_driver):
    """
    验证输入用户名名字49个字符，点击"Submit"按钮
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        try:
            ...
            assert username_length == 49, f"用户名长度应该为49个字符，实际: {username_length}"
        ...
        # 步骤13: 验证用户名显示为完整的49个字符
        current_step = "步骤13: 验证用户名显示为完整的49个字符"
        print(f"🔄 {current_step}")
        try:
            # 尝试多个可能的选择器
            username_selectors = [
            ...
            # 期望显示格式: "Hi, " + 完整的49个字符用户名
            expected_display = f"Hi, {username}"

            # 断言用户名显示为完整的49个字符
            assert username_display == expected_display,                 f"用户名显示不正确，期望'{expected_display}'，实际显示: {username_display}"

            # 验证显示的用户名长度恰好为49个字符（不包括"Hi, "）
            displayed_username = username_display.replace("Hi, ", "")
            assert len(displayed_username) == 49,                 f"显示的用户名长度应该为49个字符，实际: {len(displayed_username)} 个字符"

            print(f"✅ {current_step} - 完成")
            print(f"📝 输入的用户名长度: {len(username)} 个字符")
            print(f"📝 显示的用户名长度: {len(displayed_username)} 个字符（完整显示）")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102180_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102180",
            case_desc="验证输入用户名名字49个字符，点击Submit按钮",
            result=case_result,
