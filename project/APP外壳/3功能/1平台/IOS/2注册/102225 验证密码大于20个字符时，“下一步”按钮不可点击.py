...
def test_102225(setup_driver):
    """
    102225 验证密码大于20个字符时，“下一步”按钮不可点击
    1) 登出后首页 Sign Up → 进入注册页
    2) 输入邮箱（get_simple_email()），勾选隐私协议，点击Done收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码输入 Csx123456789012345678（>20 位）
    5) 规则提示检查：
       • 6-20 characters 期望红色
       • contains letters 期望灰色
       • contains numbers 期望灰色
       • Supports special characters:! @ # $ % ^ & * ( ) - _ = + \ | [ ] { } ; : / ? . , ~ > < ` 期望灰色（尝试读取颜色，读取不到则记录日志）
    6) 收起键盘（如有 Done），点击密码页 Next，断言仍停留在 set password 页（两密码框仍在）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 输入>20位密码并验证规则颜色
        current_step = "步骤8: 输入超长密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
            ...
            long_pwd = "Csx123456789012345678"
            password_input.send_keys(long_pwd)
            print(f"✅ 已输入密码: {long_pwd}")
            time.sleep(1.5)
            ...
            for rule_text, expect_color in rule_expectations.items():
                ...
                if color_val:
                    if expect_color == 'red':
                        assert ('255' in color_val or '#ff' in color_val.lower() or 'red' in color_val.lower()), \
                            f"期望红色但实际为 {color_val}"
                    elif expect_color == 'gray':
                        assert ('128' in color_val or 'gray' in color_val.lower() or 'grey' in color_val.lower() or '#8' in color_val.lower()), \
                            f"期望灰色但实际为 {color_val}"
            print("✅ 规则颜色检查完成（无法获取颜色时已记录）")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        # 步骤9: 收起键盘并点击Next，验证停留当前页面
        current_step = "步骤9: 收起键盘并点击Next，验证停留当前页面"
        print(f"🔄 {current_step}")
        try:
            try:
                done_btn = WebDriverWait(driver, 3).until(
                ...
                print("ℹ️ 未找到Done按钮，可能键盘已收起")

            next_btn_pwd = WebDriverWait(driver, 6).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Next"]'))
            )
            try:
                next_btn_pwd.click()
                print("✅ 已尝试点击密码页Next按钮")
            except Exception as click_err:
                print(f"ℹ️ Next可能禁用，点击未触发: {click_err}")
            time.sleep(2)

            pwd1 = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSecureTextField[1]'))
            )
            pwd2 = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSecureTextField[2]'))
            )
            assert pwd1.is_displayed() and pwd2.is_displayed(), "点击后未留在密码设置页面"
            print("✅ 点击后仍停留在set password页面，Next按钮不可使页面跳转")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print("🎉 测试用例102225执行成功！")
        print('✅ 密码大于20位时规则颜色符合预期，Next按钮不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102225_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102225",
            case_desc='102225 验证密码大于20个字符时，“下一步”按钮不可点击',
            result=case_result,
