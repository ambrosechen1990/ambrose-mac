...
def generate_long_email(length=80):
    """
    生成指定长度的邮箱地址
    ...
    Returns:
        str: 指定长度的邮箱地址
    """
    domain = "@163.com"  # 域名长度为8个字符
    username_length = length - len(domain)  # 用户名部分长度
    
    # 生成随机用户名（使用字母和数字）
    username = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(username_length))
    
    email = f"{username}{domain}"
    print(f"📧 生成长邮箱地址（{len(email)}个字符）: {email[:50]}...")
    return email
...
def test_102193(setup_driver):
    """
    验证注册时，邮箱名称过长
    1. 打开APP，点击首页"2注册"按钮
    2. 选择地区，勾选"用户政策、隐私协议"
    3. 输入邮箱名称80个字符
    4. 点击next按钮，断言停留在当前页面，新增提示"Please sign up using your email address"
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤6: 输入邮箱名称80个字符
        current_step = "步骤6: 输入邮箱名称80个字符"
        print(f"🔄 {current_step}")
        try:
            # 生成80个字符的邮箱地址
            email_address = generate_long_email(80)
            print(f"📝 邮箱地址长度: {len(email_address)} 个字符")
            ...
            print(f"✅ {current_step} - 完成，已输入80个字符的邮箱")
            time.sleep(2)
        except Exception as e:
        ...
        # 步骤9: 验证点击后仍停留在当前页面，页面出现提示信息
        current_step = "步骤9: 验证点击后仍停留在当前页面，页面出现提示信息"
        print(f"🔄 {current_step}")
        try:
            assert_on_signup_page(driver, timeout=5)
        ...
        print("🎉 测试用例102193执行成功！")
        print('✅ 验证注册时，邮箱名称过长：停留在当前页面并显示提示信息')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102193_failed")
