...
def test_102212(setup_driver):
    """
    102212 验证错误邮箱，无法进行下一步操作-只有特殊字符
    1. 打开APP，点击首页"2注册"按钮
    2. 进入Sign Up页面
    3. 邮箱输入只有特殊字符的错误邮箱（如：!@#$.(@gmail.com）
    4. 勾选隐私政策与用户协议
    5. 点击Next按钮
    ...
    try:
        ...
        # 步骤4: 邮箱输入只有特殊字符的错误邮箱
        current_step = "步骤4: 邮箱输入只有特殊字符的错误邮箱"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102212执行成功！")
        print('✅ 验证错误邮箱（特殊字符）勾选隐私后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102212_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102212",
            case_desc='102212 验证错误邮箱，无法进行下一步操作-只有特殊字符',
            result=case_result,
