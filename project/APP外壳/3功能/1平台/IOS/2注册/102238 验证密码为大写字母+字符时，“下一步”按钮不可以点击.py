...
def test_102238(setup_driver):
    """
    102238 验证密码为大写字母+字符时，“下一步”按钮不可以点击
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码输入 "!@#*XINGMAI"（大写+特殊字符），规则期望：
       • 6-20 characters 灰色
       • contains letters 灰色
       • contains numbers 红色
       • Supports special characters... 灰色（尝试读取颜色，读取不到仅记录）
    5) Done 收起键盘，点击密码页 Next，断言仍停留在 set password 页（两密码框仍存在，未跳转）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 输入大写+特殊字符密码并验证规则颜色
        current_step = "步骤8: 输入大写+特殊字符密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102238执行成功！")
        print('✅ 大写+特殊字符密码规则提示符合预期，Next不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102238_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102238",
            case_desc='102238 验证密码为大写字母+字符时，“下一步”按钮不可以点击',
            result=case_result,
