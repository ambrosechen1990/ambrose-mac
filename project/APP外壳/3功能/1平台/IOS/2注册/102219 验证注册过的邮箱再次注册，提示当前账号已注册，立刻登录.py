...
def test_102219(setup_driver):
    """
    102219 验证注册过的邮箱再次注册，提示当前账号已注册，立刻登录
    1) 首页点击 Sign Up 进入注册页
    2) 输入已注册邮箱（示例：haoc51888@gmail.com）
    3) 勾选隐私政策/用户协议
    4) 点击 Next，弹出提示 “This email is already registered. Ready to sign in now?”
    5) 点击 Confirm，跳转登录页，出现 “Don't have an account? Create One”
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤4: 输入已注册邮箱
        current_step = "步骤4: 输入已注册邮箱"
        print(f"🔄 {current_step}")
        try:
            email_registered = "haoc51888@gmail.com"
            email_input = WebDriverWait(driver, 8).until(
                EC.element_to_be_clickable((AppiumBy.IOS_PREDICATE, 'type == "XCUIElementTypeTextField"'))
            )
            email_input.clear()
            email_input.send_keys(email_registered)
            print(f"✅ {current_step} - 完成，输入邮箱: {email_registered}")
            time.sleep(2)
        except Exception as e:
        ...
        # 步骤7: 验证弹框提示已注册
        current_step = "步骤7: 验证弹框提示已注册"
        print(f"🔄 {current_step}")
        try:
            alert_text_elem = WebDriverWait(driver, 8).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="This email is already registered. Ready to sign in now?"]'))
            )
            assert alert_text_elem.is_displayed(), "未出现已注册提示弹框"
            confirm_btn_in_alert = WebDriverWait(driver, 4).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Confirm"]'))
            )
            cancel_btn_in_alert = WebDriverWait(driver, 4).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Cancel"]'))
            )
            assert confirm_btn_in_alert.is_displayed(), "弹框中未显示 Confirm 按钮"
            assert cancel_btn_in_alert.is_displayed(), "弹框中未显示 Cancel 按钮"
            print("✅ 弹框提示已注册: This email is already registered. Ready to sign in now?")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: 未找到已注册提示弹框 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        # 步骤8: 点击Confirm进入登录页
        current_step = "步骤8: 点击Confirm进入登录页"
        print(f"🔄 {current_step}")
        try:
            ...
            fail_reason = f"{current_step}失败: 无法点击Confirm按钮 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        # 步骤9: 验证跳转到登录页面
        current_step = "步骤9: 验证跳转到登录页面"
        print(f"🔄 {current_step}")
        try:
            sign_in_text_elem = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Sign In"]'))
            )
            assert sign_in_text_elem.is_displayed(), "未找到登录页面 Sign In 文案，可能未跳转登录页"
            print(f"✅ {current_step} - 完成，已跳转到登录页面")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: 未成功跳转到登录页面 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        print("🎉 测试用例102219执行成功！")
        print('✅ 已注册邮箱再次注册，弹框提示并点击Confirm跳转登录页成功')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102219_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102219",
            case_desc='102219 验证注册过的邮箱再次注册，提示当前账号已注册，立刻登录',
            result=case_result,
