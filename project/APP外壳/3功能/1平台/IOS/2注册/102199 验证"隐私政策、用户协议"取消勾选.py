...
def test_102199(setup_driver):
    ...
    try:
        ...
        # 步骤7: 再次点击隐私框（取消勾选）
        current_step = "步骤7: 再次点击隐私框（取消勾选）"
        print(f"🔄 {current_step}")
        try:
            # 查找已勾选状态的复选框并点击取消勾选
            checked_btn = WebDriverWait(driver, 8).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="login check selected"]'))
            )
            assert checked_btn.is_displayed(), "已勾选的隐私政策复选框存在但不可见"
            checked_btn.click()  # 点击已勾选的复选框，取消勾选
            print(f"✅ 已点击隐私政策复选框，取消勾选")
            time.sleep(2)  # 等待状态更新
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法找到或点击已勾选的复选框 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        # 步骤8: 验证页面显示隐私框未勾选
        current_step = "步骤8: 验证页面显示隐私框未勾选"
        print(f"🔄 {current_step}")
        try:
            # 验证未勾选状态的复选框存在
            ...
            assert check_btn.is_displayed(), "未勾选的隐私政策复选框应该存在"
            
            # 验证已勾选状态的复选框不存在或不可见
            try:
                checked_btn = driver.find_element(AppiumBy.XPATH, '//XCUIElementTypeButton[@name="login check selected"]')
                if checked_btn.is_displayed():
                    raise Exception("隐私政策复选框应该未勾选，但仍显示为已勾选状态")
            except:
                # 找不到已勾选状态的按钮，说明未勾选，这是预期的
                pass
            
            print(f"✅ {current_step} - 完成，验证通过：隐私框未勾选")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        # 步骤10: 验证点击后停留在当前页面
        current_step = "步骤10: 验证点击后停留在当前页面"
        print(f"🔄 {current_step}")
        try:
            ...
            # 验证未跳转到密码设置页面（通过检查密码输入框不存在）
            try:
                password_input = driver.find_element(AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSecureTextField[1]')
                if password_input.is_displayed():
                    raise Exception("不应该跳转到密码设置页面，但检测到密码输入框存在")
            except:
                # 找不到密码输入框，说明未跳转，这是预期的
                print("✅ 验证通过：未跳转到密码设置页面")
            
            print(f"✅ {current_step} - 完成")
            print(f"✅ 验证通过：点击Next按钮后停留在当前页面，未跳转")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print('✅ 验证"隐私政策、用户协议"取消勾选：勾选后再次点击取消勾选，点击Next按钮后停留在当前页面')
        time.sleep(2)
