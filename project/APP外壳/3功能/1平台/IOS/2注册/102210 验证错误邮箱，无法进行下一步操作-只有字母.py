...
def test_102210(setup_driver):
    """
    102210 验证错误邮箱，无法进行下一步操作-只有字母
    1. 打开APP，点击首页"2注册"按钮
    2. 进入Sign Up页面
    3. 邮箱输入只有字母的错误邮箱（如：abcdefg）
    4. 勾选隐私政策与用户协议
    5. 点击Next按钮
    ...
    try:
        ...
        # 步骤4: 邮箱输入只有字母的错误邮箱
        current_step = "步骤4: 邮箱输入只有字母的错误邮箱"
        print(f"🔄 {current_step}")
        try:
            wrong_email = "abcdefg"
        ...
        print("🎉 测试用例102210执行成功！")
        print('✅ 验证错误邮箱（仅字母）勾选隐私后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102210_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102210",
            case_desc='102210 验证错误邮箱，无法进行下一步操作-只有字母',
            result=case_result,
