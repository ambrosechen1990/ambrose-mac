...
def test_102237(setup_driver):
    """
    102237 验证密码为小写字母+字符时，“下一步”按钮不可以点击
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码输入 "!@#&xingmai"（小写+特殊字符），规则期望：
       • 6-20 characters 灰色
       • contains letters 灰色
       • contains numbers 红色
    ...
    try:
        ...
        # 步骤8: 输入小写+特殊字符密码并验证规则颜色
        current_step = "步骤8: 输入小写+特殊字符密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102237执行成功！")
        print('✅ 小写+特殊字符密码规则提示符合预期，Next不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102237_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102237",
            case_desc='102237 验证密码为小写字母+字符时，“下一步”按钮不可以点击',
            result=case_result,
