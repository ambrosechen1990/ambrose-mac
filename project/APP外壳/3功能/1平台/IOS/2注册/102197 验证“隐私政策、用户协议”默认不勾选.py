...
def test_102197(setup_driver):
    ...
    try:
        ...
        # 步骤7: 点击Next按钮（不勾选隐私协议）
        current_step = "步骤7: 点击Next按钮（不勾选隐私协议）"
        print(f"🔄 {current_step}")
        try:
            # Next 可能处于禁用态，element_to_be_clickable 会超时；
            # 这里采用 presence + 多selector，尽最大努力触发点击，然后在下一步断言提示出现即可。
            next_selectors = [
            ...
            try:
                ...
                # 禁用态点击可能抛异常，但不应直接判失败；下一步用提示文案来判定行为
        ...
        # 步骤8: 断言提示出现（即判成功）
        current_step = "步骤8: 断言提示出现（即判成功）"
        print(f"🔄 {current_step}")
        try:
            # 通过条件：出现提示文案即可（默认未勾选由业务前提保证，不再做额外强断言）
            ...
            error_message = err.get_attribute("name") or ""
