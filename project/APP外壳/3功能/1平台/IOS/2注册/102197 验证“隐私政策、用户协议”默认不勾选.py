    ...
    try:
        ...
        try:
            ...
            search_field = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeSearchField[@name="Search"]'))
            )
            assert search_field.is_displayed(), "搜索框存在但不可见"
            print(f"✅ 确认已跳转至国家选择页面")
            ...
            # 选择China
            china_option = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="China"]'))
            )
            assert china_option.is_displayed(), "China选项存在但不可见"
            china_option.click()
            print(f"✅ 已选择China")
        ...
        # 步骤8: 点击Next按钮
        current_step = "步骤8: 点击Next按钮"
        print(f"🔄 {current_step}")
        try:
            # 记录点击前的页面状态：确认隐私协议元素存在
            privacy_element_before = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeOther[@name="I have read and understood the Privacy Policy and agree to the User Agreement."]'))
            )
        ...
        # 步骤9: 验证点击后仍停留在当前页面，显示页面提示
        current_step = "步骤9: 验证点击后仍停留在当前页面，显示页面提示"
        print(f"🔄 {current_step}")
        try:
            ...
            print("✅ 验证通过：点击Next按钮后仍停留在当前页面")
            
            # 验证页面显示提示信息"Please confirm and tick the Privacy Policy and User Agreement"
            # 尝试多种可能的提示信息文本（英文和中文）
            error_message_selectors = [
                '//XCUIElementTypeStaticText[@name="Please confirm and tick the Privacy Policy and User Agreement"]',
                '//XCUIElementTypeStaticText[contains(@name, "Please confirm and tick")]',
                '//XCUIElementTypeStaticText[contains(@name, "Privacy Policy")]',
                '//XCUIElementTypeStaticText[contains(@name, "User Agreement")]',
                '//XCUIElementTypeStaticText[@name="请确认并勾选隐私政策与用户协议"]',
                '//XCUIElementTypeStaticText[contains(@name, "请确认并勾选")]',
                '//XCUIElementTypeStaticText[contains(@name, "隐私政策")]',
                '//XCUIElementTypeStaticText[contains(@name, "用户协议")]',
            ]
                ...
                try:
                    ...
                    for elem in elements:
                        if elem.is_displayed():
                            error_text = elem
                            error_message = elem.get_attribute("name")
                            print(f"✅ 找到提示信息元素，使用选择器: {selector}")
                            break
                    if error_text:
                        break
                except:
            ...
            if not error_text:
                raise Exception("无法找到提示信息元素，请检查页面元素")
            
            print(f"📝 提示信息内容: {error_message}")
            
            # 验证提示信息包含关键内容（支持英文和中文）
            assert ("Please confirm and tick" in error_message or "Privacy Policy" in error_message or "User Agreement" in error_message or 
                    "请确认并勾选" in error_message or "隐私政策" in error_message or "用户协议" in error_message), \
                f"提示信息不正确，期望包含'Please confirm and tick'或'Privacy Policy'或'User Agreement'或'请确认并勾选'或'隐私政策'或'用户协议'，实际显示: {error_message}"
            
            print(f"✅ {current_step} - 完成")
            print(f"✅ 验证通过：页面显示提示信息: {error_message}")
            time.sleep(2)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print("🎉 测试用例102197执行成功！")
        print('✅ 102197 "隐私政策、用户协议"默认不勾选')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102197_failed")
