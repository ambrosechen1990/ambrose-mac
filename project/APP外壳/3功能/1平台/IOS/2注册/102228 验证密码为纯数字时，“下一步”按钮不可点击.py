...
def test_102228(setup_driver):
    """
    102228 验证密码为纯数字时，“下一步”按钮不可点击
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email），勾选隐私，点击Done收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码输入纯数字 123456
       规则期望：
         • 6-20 characters 灰色
         • contains letters 红色
         • contains numbers 灰色
         • Supports special characters ... 灰色
    5) 收起键盘（Done），点击密码页 Next，断言仍停留在 set password 页（无法跳转）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 输入纯数字密码并验证规则颜色
        current_step = "步骤8: 输入纯数字密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        # 步骤9: 点击Next（预期不可跳转），验证仍留在set password页面
        current_step = "步骤9: 点击Next验证仍停留在set password页面"
        print(f"🔄 {current_step}")
        try:
            ...
            try:
                next_btn_pwd.click()
                print("✅ 已尝试点击密码页Next按钮（应无跳转）")
            except Exception as click_err:
                print(f"ℹ️ Next可能禁用，点击未触发: {click_err}")
            time.sleep(2)

            # 断言仍在set password页面（两个密码框仍存在）
        ...
        print("🎉 测试用例102228执行成功！")
        print('✅ 纯数字密码规则提示符合预期，Next不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102228_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102228",
            case_desc='102228 验证密码为纯数字时，“下一步”按钮不可点击',
            result=case_result,
