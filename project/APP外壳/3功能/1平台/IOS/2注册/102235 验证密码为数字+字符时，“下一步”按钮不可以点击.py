...
def test_102235(setup_driver):
    """
    102235 验证密码为数字+字符时，“下一步”按钮不可以点击
    1) 登出后首页 Sign Up → 注册页
    ...
    4) 密码与确认密码输入 "!@#$%^&*()-==+\\'l[};:/?...~><`12"
       ...
       • contains letters 红
       • contains numbers 灰
       • Supports special characters... 灰（尝试取色，取不到仅记录）
    5) Done 收起键盘，点击密码页 Next，断言仍停留在 set password 页（两密码框仍在，未跳转）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤8: 输入数字+字符密码并验证规则颜色
        current_step = "步骤8: 输入数字+字符密码并验证规则颜色"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102235执行成功！")
        print('✅ 数字+特殊字符密码规则提示符合预期，Next不可跳转')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102235_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102235",
            case_desc='102235 验证密码为数字+字符时，“下一步”按钮不可以点击',
            result=case_result,
