...
def test_102214(setup_driver):
    """
    102214 验证错误邮箱，无法进行下一步操作-a@@b.（ab代表n个数字或字母）
    1. 打开APP，点击首页"2注册"按钮
    2. 进入Sign Up页面
    3. 邮箱输入错误格式（如：asd@@gmial.com）
    4. 勾选隐私政策与用户协议
    5. 点击Next按钮
    ...
    try:
        ...
        # 步骤4: 邮箱输入错误格式 asd@@gmial.com
        current_step = "步骤4: 邮箱输入错误格式"
        print(f"🔄 {current_step}")
        try:
            wrong_email = "asd@@gmial.com"
            ...
            time.sleep(1)  # 等待输入完成
            # 验证输入是否正确
            input_value = email_input.get_attribute("value") or ""
            print(f"📝 输入框实际值: '{input_value}'")
            if input_value != wrong_email:
                print(f"⚠️ 输入值不匹配！期望: '{wrong_email}', 实际: '{input_value}'")
                # 尝试重新输入
                email_input.clear()
                time.sleep(0.5)
                email_input.send_keys(wrong_email)
        ...
        print("🎉 测试用例102214执行成功！")
        print('✅ 验证错误邮箱（a@@b型）勾选隐私后点击Next，停留当前页并提示“Please sign up using your email address”')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        screenshot_path = save_failure_screenshot(driver, "test_102214_failed", run_dir=RUN_DIR) or ""
        ...
        write_report(
            ...
            platform="ios",
            case_id="102214",
            case_desc='102214 验证错误邮箱，无法进行下一步操作-a@@b.（ab代表n个数字或字母）',
            result=case_result,
