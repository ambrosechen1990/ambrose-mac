...
def test_102227(setup_driver):
    """
    102227 验证密码等于20个字符时，“下一步”按钮可点击
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码与确认密码输入 Csx1234567Csx1234567（20 位）
    5) 再次 Done 收起键盘，点击密码页 Next 进入用户设置页
    6) 点击 Submit，验证进入主页面（检测到 mine 元素或等价标识）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤10: 用户名页面点击Skip，验证跳转主页面（mine元素）
        ...
        print("🎉 测试用例102227执行成功！")
        print('✅ 密码等于20位时可继续，提交后成功进入主页面（mine元素）')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102227_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102227",
            case_desc='102227 验证密码等于20个字符时，“下一步”按钮可点击',
            result=case_result,
