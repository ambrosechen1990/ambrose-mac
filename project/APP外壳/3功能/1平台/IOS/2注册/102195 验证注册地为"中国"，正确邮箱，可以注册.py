...
def switch_country(driver, country_name: str):
    ...
    Args:
        driver: WebDriver实例
        country_name: 国家名称，如"China"或"中国"
...
def test_102195(setup_driver):
    """
    102195 验证注册地为"中国"，正确邮箱，可以注册
    流程（按最新梳理）：
    1. 打开APP（若已登录先登出）
    2. 点击 Sign Up 进入注册页
    3. 国家切为 china
    4. 输入邮箱（value="Email"，邮箱使用共用脚本生成）
    5. 勾选隐私协议（login check normal）
    6. Next → 设置密码（value="Password"/"Retype Password"）
    7. Next → 设置用户名（value="Username" 输入 tank）→ Done
    8. Submit → 断言出现 home sel
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        print(f"🔄 {current_step}")
        try:
            email_address = get_simple_email()  # 调用共用脚本生成邮箱
            email_selectors = [
        ...
        try:
            pwd_value_selectors = [
                (AppiumBy.XPATH, '//XCUIElementTypeSecureTextField[@value="Password"]'),
                (AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSecureTextField[1]'),
            ]
            password_input = None
            last_err = None
            for by, locator in pwd_value_selectors:
                try:
                    password_input = WebDriverWait(driver, 8).until(EC.element_to_be_clickable((by, locator)))
                    if password_input:
                        break
                except Exception as e:
                    last_err = e
            if not password_input:
                raise Exception(f"未找到密码输入框: {last_err}")
            ...
            # 确认密码
            retype_selectors = [
                (AppiumBy.XPATH, '//XCUIElementTypeSecureTextField[@value="Retype Password"]'),
                (AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeSecureTextField[2]'),
            ]
            retype_password_input = None
            last_err = None
            for by, locator in retype_selectors:
                try:
                    retype_password_input = WebDriverWait(driver, 8).until(EC.element_to_be_clickable((by, locator)))
                    if retype_password_input:
                        break
                except Exception as e:
                    last_err = e
            if not retype_password_input:
                raise Exception(f"未找到确认密码输入框: {last_err}")
        ...
        # 步骤11: 点击Next按钮进入用户名设置页面
        current_step = "步骤11: 点击Next按钮进入用户名设置页面"
        print(f"🔄 {current_step}")
        try:
            next_btn = WebDriverWait(driver, 8).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Next"]'))
        ...
        # 步骤12: 输入用户名（value="Username" 输入 tank）
        current_step = '步骤12: 输入用户名（value="Username"）'
        print(f"🔄 {current_step}")
        try:
            username_selectors = [
            ...
            username_input = None
            last_err = None
            for by, locator in username_selectors:
                try:
                    username_input = WebDriverWait(driver, 8).until(EC.element_to_be_clickable((by, locator)))
                    if username_input:
                        break
                except Exception as e:
                    last_err = e
            if not username_input:
                raise Exception(f"未找到用户名输入框: {last_err}")
            username_input.clear()
            username_input.send_keys("tank")
            print(f"✅ {current_step} - 完成")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法找到或输入用户名 - {str(e)}"
            print(f"❌ {fail_reason}")
            raise

        # 步骤13: 点击Done按钮收起键盘
        current_step = "步骤13: 点击Done按钮收起键盘"
        print(f"🔄 {current_step}")
        try:
            done_btn = WebDriverWait(driver, 5).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Done"]'))
            ...
            print(f"ℹ️ {current_step} - Done按钮未出现或无法点击，跳过: {str(e)}")
            time.sleep(1)

        # 步骤14: 点击Submit按钮完成注册
        current_step = "步骤14: 点击Submit按钮完成注册"
        print(f"🔄 {current_step}")
        try:
            # 点击Submit按钮（不输入用户名）- 多定位兜底
            submit_selectors = [
                (AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Submit"]'),
                (AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Submit "]'),
                (AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Submit"]'),
                (AppiumBy.XPATH, '//XCUIElementTypeOther[@name="Submit"]'),
            ]
            submit_btn = None
            last_err = None
            for by, locator in submit_selectors:
                try:
                    submit_btn = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((by, locator)))
                    if submit_btn:
                        break
                except Exception as e:
                    last_err = e
                    continue
            if not submit_btn:
                raise Exception(f"未找到可点击的 Submit 按钮: {last_err}")
            submit_btn.click()
            print(f"✅ {current_step} - 完成")
        ...
        # 步骤15: 验证注册成功，跳转到APP主页面（断言 home sel）
        current_step = "步骤15: 验证注册成功，跳转到APP主页面"
        print(f"🔄 {current_step}")
        try:
        ...
        print("🎉 测试用例102195执行成功！")
        print('✅ 验证注册地为"中国"，正确邮箱，可以注册：注册成功并跳转到主页面')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102195_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102195",
            case_desc='102195 验证注册地为"中国"，正确邮箱，可以注册',
            result=case_result,
