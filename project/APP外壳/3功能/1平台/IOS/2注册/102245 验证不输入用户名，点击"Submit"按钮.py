...
def test_102245(setup_driver):
    """
    102245 验证不输入用户名，点击"Submit"按钮
    ...
    6) 不输入用户名，直接点击 Submit 按钮
    7) 验证仍停留在当前页面，有元素 Personal Information（通过首页标识元素验证）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤10: 不输入用户名，点击Submit并验证仍停留在页面
        current_step = "步骤10: 不输入用户名，点击Submit并验证仍停留在页面"
        print(f"🔄 {current_step}")
        try:
            ...
            assert submit_btn is not None, "未找到 Submit 按钮/文案元素"

            try:
                submit_btn.click()
            except Exception:
                rect = submit_btn.rect or {}
            ...
            print("✅ 已点击Submit按钮（未输入用户名）")
            time.sleep(2)

            # 验证仍停留在Personal Information页面
            personal_info_text_after = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Personal Information"]'))
            )
            assert personal_info_text_after.is_displayed(), "点击Submit后Personal Information文本元素应仍存在，说明仍停留在当前页面"
            print("✅ 点击Submit后仍停留在Personal Information页面")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print("🎉 测试用例102245执行成功！")
        print('✅ 不输入用户名点击Submit后仍停留在Personal Information页面')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102245_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102245",
            case_desc='102245 验证密码等于6个字符时，“下一步”按钮可点击',
            result=case_result,
