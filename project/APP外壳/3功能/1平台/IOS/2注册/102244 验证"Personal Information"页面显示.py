...
def test_102244(setup_driver):
    """
    102244 验证"Personal Information"页面显示
    ...
    6) 验证页面有 Skip 和 Submit 元素（通过首页标识元素验证）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤10: 验证Personal Information页面元素
        current_step = "步骤10: 验证Personal Information页面元素"
        print(f"🔄 {current_step}")
        try:
            # 验证 Skip 元素
            skip_selectors = [
            ...
            if not skip_elem:
                raise Exception("未找到Skip元素，可能未进入Personal Information页面")

            # 验证 Submit 按钮
            submit_btn = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeButton[@name="Submit"]'))
            )
            assert submit_btn.is_displayed(), "Submit按钮存在但不可见"
            print("✅ 找到Submit按钮")

            print("✅ Personal Information页面元素验证通过")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print("🎉 测试用例102226执行成功！")
        print('✅ Personal Information页面显示正确，包含Skip和Submit元素')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102244_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102244",
            case_desc='102244 验证密码等于6个字符时，“下一步”按钮可点击',
            result=case_result,
