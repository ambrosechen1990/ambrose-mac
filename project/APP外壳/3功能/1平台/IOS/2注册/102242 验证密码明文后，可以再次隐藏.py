...
def test_102242(setup_driver):
    """
    102242 验证密码明文后，可以再次隐藏
    1) 登出后首页 Sign Up → 注册页
    2) 输入邮箱（get_simple_email()），勾选隐私，点击 Done 收起键盘
    3) 点击 Next 进入 set password 页
    4) 密码框输入 Csx150128
    5) 点击显示密码按钮 //XCUIElementTypeButton[@name="login pwd hide"] 显示明文
    6) 再次点击同一按钮隐藏密码，断言变回密文（值不等于明文，类型为 SecureTextField）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        try:
            ...
            field_type_plain = plain_password_input.get_attribute("type") or ""
            print(f"📝 明文时value: '{value_plain}', type: '{field_type_plain}'")
            assert value_plain == pwd, f"应显示明文，当前: '{value_plain}'"

            # 第二次点击同按钮，隐藏密码
            show_btn.click()
            print("✅ 第二次点击显示密码按钮（隐藏）")
            time.sleep(1)
            ...
            if value_hidden:
                assert len(str(value_hidden)) >= len(pwd), "隐藏时密文长度应不少于密码长度"
            print("✅ 明文后再次隐藏成功")
            time.sleep(1)
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
        ...
        print("🎉 测试用例102242执行成功！")
        print('✅ 明文显示后可再次隐藏密文校验通过')
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102242_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102242",
            case_desc='102242 验证密码明文后，可以再次隐藏',
            result=case_result,
