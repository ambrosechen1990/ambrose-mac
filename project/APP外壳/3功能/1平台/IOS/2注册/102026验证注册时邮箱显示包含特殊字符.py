...
def test_102026(setup_driver):
    """
    验证注册时邮箱包含合法特殊字符时，可以继续注册。
    输入形如 1+2-3.4%5_681111@163.com 的邮箱，勾选协议后点击 Next，
    断言成功进入设置密码页面。
    """
    driver = setup_driver
    case_result = "success"
    ...
    try:
        ...
        # 步骤2-6: 输入邮箱 -> 勾选协议 -> Next -> 验证进入密码页（若后端提示邮箱已被占用，自动换邮箱重试）
        current_step = "步骤2-6: 输入邮箱并进入设置密码页面"
        print(f"🔄 {current_step}")
        try:
            max_attempts = 6
            last_email = ""
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                # 6) 验证进入 set password 页面（短等待；失败则换邮箱重试）
                try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102026_failed")
