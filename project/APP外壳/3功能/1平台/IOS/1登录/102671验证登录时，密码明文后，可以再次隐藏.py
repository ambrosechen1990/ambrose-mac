...
from ios_sign_in_locators import IOS_SIGN_IN_PASSWORD_VISIBILITY_TOGGLE_XPATH
from ios_sign_in_helpers import (
    resolve_sign_in_password_input,
    wait_sign_in_password_hidden,
    wait_sign_in_password_visible,
)
...
def test_102671(setup_driver):
    ...
    try:
        ...
        try:
            ...
            # 验证密码被隐藏（检查输入框类型是否为SecureTextField）
        ...
        try:
            ...
            eye_btn = WebDriverWait(driver, 5).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, IOS_SIGN_IN_PASSWORD_VISIBILITY_TOGGLE_XPATH))
            )
            eye_btn.click()
            print("    ✅ 已点击眼睛按钮")
            time.sleep(1)
            
            visible_state = wait_sign_in_password_visible(driver, password)
            print(f"    ✅ 密码已显示明文（状态: {visible_state}）")
        ...
        try:
            ...
            hidden_state = wait_sign_in_password_hidden(driver, password)
            print(f"    ✅ 验证密码被隐藏成功（状态: {hidden_state}）")
