...
def test_102660(setup_driver):
    """
    验证输入用邮箱和密码，不勾选用户协议和隐私政策
    1. 重置APP，检测登录状态，如果已登录则登出
    2. 点击登录按钮进入Sign in页面
    3. 点击邮箱框，输入邮箱：haoc51888@gmail.com
    4. 点击密码框，输入密码：Csx150128
    5. 点击键盘done，收起键盘
    6. 点击登录按钮（不勾选用户协议和隐私政策）
    7. 断言页面显示错误提示：Please confirm and tick the Privacy Policy and User Agreement
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤6: 点击登录按钮（不勾选用户协议和隐私政策）
        current_step = "步骤6: 点击登录按钮（不勾选用户协议和隐私政策）"
        print(f"🔄 {current_step}")
        try:
        ...
        try:
            ...
            error_message = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Please confirm and tick the Privacy Policy and User Agreement"]'))
            )
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102660_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102660",
            case_desc="验证输入用邮箱和密码，不勾选用户协议和隐私政策",
            result=case_result,
