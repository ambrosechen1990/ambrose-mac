...
from ios_sign_in_locators import IOS_SIGN_IN_EMAIL_CONTAINER_XPATH
from ios_sign_in_helpers import resolve_sign_in_password_input
...
def test_102653(setup_driver):
    """
    验证不输入邮箱和输入密码，勾选用户协议和隐私政策，点击登录
    1. 重置APP，检测登录状态，如果已登录则登出
    2. 点击登录按钮进入Sign in页面
    3. 不输入邮箱（保持为空）
    4. 点击密码框，输入密码：Csx150128
    5. 点击键盘done，收起键盘
    6. 点击协议勾选框（勾选用户协议和隐私政策）
    7. 点击登录按钮
    8. 断言页面显示错误提示：Please enter a valid email address.
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤3: 验证邮箱输入框为空（不输入邮箱）
        current_step = "步骤3: 验证邮箱输入框为空（不输入邮箱）"
        print(f"🔄 {current_step}")
        try:
            # 验证邮箱输入框为空
            email_input = driver.find_elements(AppiumBy.XPATH, IOS_SIGN_IN_EMAIL_CONTAINER_XPATH)
            if email_input:
                email_value = email_input[0].get_attribute("value") or ""
                if email_value:
                    # 如果邮箱输入框有内容，清空它
                    email_input[0].clear()
                    print(f"    ℹ️ 邮箱输入框有内容，已清空: {email_value}")
                else:
                    print(f"    ✅ 邮箱输入框为空")
            
            print(f"✅ {current_step} - 完成，邮箱输入框为空")
        except Exception as e:
            print(f"    ⚠️ 验证邮箱输入框为空时出错（可忽略）: {e}")
            time.sleep(0.5)
        ...
        # 步骤5: 点击键盘done，收起键盘
        current_step = "步骤5: 点击键盘Done，收起键盘"
        print(f"🔄 {current_step}")
        try:
        ...
        # 步骤6: 点击协议勾选框（勾选用户协议和隐私政策）
        current_step = "步骤6: 点击协议勾选框（勾选用户协议和隐私政策）"
        print(f"🔄 {current_step}")
        try:
        ...
        # 步骤7: 点击登录按钮（已勾选用户协议和隐私政策，但不输入邮箱但输入了密码）
        current_step = "步骤7: 点击登录按钮（已勾选用户协议和隐私政策，但不输入邮箱但输入了密码）"
        print(f"🔄 {current_step}")
        try:
        ...
        try:
            ...
            error_message = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Please enter a valid email address."]'))
            )
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102653_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102653",
            case_desc="验证不输入邮箱和输入密码，勾选用户协议和隐私政策，点击登录",
            result=case_result,
