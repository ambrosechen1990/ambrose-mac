...
def test_102652(setup_driver):
    ...
    2. 点击登录按钮进入Sign in页面
    3. 点击邮箱框，输入邮箱：haoc51888chenhao@gmail.com（未注册的邮箱）
    4. 点击密码框，输入密码：Csx150128
    5. 点击键盘done，收起键盘
    6. 点击协议勾选框
    7. 点击登录按钮
    8. 断言页面显示错误提示：This email is not registered. Please check and re-enter.
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤3: 点击邮箱框，输入邮箱（未注册的邮箱）
        current_step = "步骤3: 点击邮箱框，输入邮箱（未注册的邮箱）"
        print(f"🔄 {current_step}")
        try:
            email_address = "haoc51888chenhao@gmail.com"
            # 点击邮箱标签
        ...
        try:
            ...
            error_message = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="This email is not registered. Please check and re-enter."]'))
            )
