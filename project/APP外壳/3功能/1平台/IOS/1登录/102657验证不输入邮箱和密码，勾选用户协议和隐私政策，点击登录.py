...
def test_102657(setup_driver):
    """
    验证不输入邮箱和密码，勾选用户协议和隐私政策，点击登录
    1. 重置APP，检测登录状态，如果已登录则登出
    2. 点击登录按钮进入Sign in页面
    3. 不输入邮箱和密码（保持为空）
    4. 点击协议勾选框（勾选用户协议和隐私政策）
    5. 点击登录按钮
    6. 断言页面显示错误提示：Please enter a valid email address.
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤3: 验证邮箱和密码输入框为空（不输入任何内容）
        current_step = "步骤3: 验证邮箱和密码输入框为空"
        print(f"🔄 {current_step}")
        try:
            # 验证邮箱输入框为空
            ...
            print(f"✅ {current_step} - 完成，邮箱和密码输入框均为空")
        except Exception as e:
            print(f"    ⚠️ 验证输入框为空时出错（可忽略）: {e}")
            time.sleep(0.5)

        # 步骤4: 点击协议勾选框（勾选用户协议和隐私政策）
        current_step = "步骤4: 点击协议勾选框（勾选用户协议和隐私政策）"
        print(f"🔄 {current_step}")
        try:
        ...
        # 步骤5: 点击登录按钮（不输入邮箱和密码，但已勾选用户协议和隐私政策）
        current_step = "步骤5: 点击登录按钮（不输入邮箱和密码，但已勾选用户协议和隐私政策）"
        print(f"🔄 {current_step}")
        try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102657_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102657",
            case_desc="验证不输入邮箱和密码，勾选用户协议和隐私政策，点击登录",
            result=case_result,
