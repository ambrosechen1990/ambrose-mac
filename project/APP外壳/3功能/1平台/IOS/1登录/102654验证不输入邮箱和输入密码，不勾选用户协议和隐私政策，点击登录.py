...
def test_102654(setup_driver):
    """
    验证不输入邮箱和输入密码，不勾选用户协议和隐私政策，点击登录
    ...
    try:
        ...
        # 步骤6: 点击登录按钮（不勾选用户协议和隐私政策，不输入邮箱但输入了密码）
        current_step = "步骤6: 点击登录按钮（不勾选用户协议和隐私政策，不输入邮箱但输入了密码）"
        print(f"🔄 {current_step}")
        try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102654_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102654",
            case_desc="验证不输入邮箱和输入密码，不勾选用户协议和隐私政策，点击登录",
            result=case_result,
