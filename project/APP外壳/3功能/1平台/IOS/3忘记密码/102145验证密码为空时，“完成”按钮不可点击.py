...
def test_102145(setup_driver):
    """
    102145 验证密码为空时，“完成”(Submit)按钮不可点击
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        current_step = "步骤7: 不输入密码，断言Submit/完成按钮不可点击"
        print(f"🔄 {current_step}")
        _assert_submit_disabled_or_no_effect(driver)
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102145执行成功！")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102145_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102145",
            case_desc='102145 验证密码为空时，“完成”按钮不可点击',
            result=case_result,
