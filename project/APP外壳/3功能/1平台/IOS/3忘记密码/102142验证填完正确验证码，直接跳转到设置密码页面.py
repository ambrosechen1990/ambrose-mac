...
def _assert_on_set_password_page(driver, timeout_s: int = 20):
    """
    断言跳转到设置密码页（出现 Set Password 文案/标题）。
    """
    pred = (
...
def test_102142(setup_driver):
    """
    102142 验证填完正确验证码，直接跳转到设置密码页面
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        current_step = "步骤6: 输入正确验证码并断言跳转Set Password"
        print(f"🔄 {current_step}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102142_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102142",
            case_desc="102142 验证填完正确验证码，直接跳转到设置密码页面",
            result=case_result,
