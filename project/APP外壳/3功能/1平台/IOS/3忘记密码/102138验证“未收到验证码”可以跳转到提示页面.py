...
def _assert_on_verification_page(driver, timeout: int = 15):
    def _ok(d):
        ...
        try:
            assists = d.find_elements(
                AppiumBy.IOS_PREDICATE,
                '(type == "XCUIElementTypeStaticText" OR type == "XCUIElementTypeButton") AND '
                '(name CONTAINS "verification code" OR label CONTAINS "verification code" OR value CONTAINS "verification code" OR '
                'name CONTAINS "验证码" OR label CONTAINS "验证码" OR value CONTAINS "验证码")',
            )
            for e in assists:
                try:
                    if e.is_displayed():
...
def _collect_visible_texts(driver):
    texts = []
    try:
...
def _assert_unreceived_code_help_page(driver, timeout_s: int = 12):
    expected_snippets = [
        "Please verify that the email address you entered is correct",
        "Please check your spam or junk folder in your email",
        "If you still haven't received the verification code",
    ]

    def _all_present(d):
        texts = _collect_visible_texts(d)
        merged = "\n".join(texts)
        return all(s in merged for s in expected_snippets)

    WebDriverWait(driver, timeout_s).until(lambda d: _all_present(d))
...
def test_102138(setup_driver):
    """
    102138 验证“未收到验证码”可以跳转到提示页面
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        current_step = "步骤5: 点击未收到验证码并跳转到提示页"
        print(f"🔄 {current_step}")
        _click_with_tap_fallback(
            driver,
            selectors=[
                (
                    AppiumBy.XPATH,
                    '//XCUIElementTypeStaticText[@name="Can\'t receive verification code?"]',
                ),
                (
                    AppiumBy.XPATH,
                    '//XCUIElementTypeButton[@name="Can\'t receive verification code?"]',
                ),
                (
                    AppiumBy.IOS_PREDICATE,
                    '(type == "XCUIElementTypeStaticText" OR type == "XCUIElementTypeButton") AND '
                    '(name CONTAINS "receive verification code" OR label CONTAINS "receive verification code" OR value CONTAINS "receive verification code")',
                ),
            ],
            timeout_each=12,
        )
        _assert_unreceived_code_help_page(
            driver,
            timeout_s=int(os.environ.get("HELP_PAGE_ASSERT_TIMEOUT_S", "12")),
        )
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102138执行成功！")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102138_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102138",
            case_desc='102138 验证“未收到验证码”可以跳转到提示页面',
            result=case_result,
