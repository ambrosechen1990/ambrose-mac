...
def _check_password_rules_for_too_long(driver):
    """
    参考注册端规则校验：
    - 6-20 characters 应为红色
    - contains letters / contains numbers / special characters 应为灰色
    """
    rule_expectations = {
        '• 6-20 characters': 'red',
...
def test_102147(setup_driver):
    """
    102147 验证密码大于20个字符时，“完成”(Submit)按钮不可点击（忘记密码链路）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    long_pwd = os.environ.get("LONG_PASSWORD", "Csx123456789012345678")  # > 20
    ...
    try:
        ...
        try:
            ...
            pass
        pwd2.send_keys(long_pwd)
        time.sleep(0.6)
        _dismiss_keyboard(driver)
        print(f"✅ {current_step} - 完成，密码: {long_pwd}")

        current_step = "步骤8: 校验密码规则颜色（超长场景）"
        print(f"🔄 {current_step}")
        _check_password_rules_for_too_long(driver)
        print(f"✅ {current_step} - 完成")

        current_step = "步骤9: 点击Submit并断言不可生效"
        print(f"🔄 {current_step}")
        _assert_submit_disabled_or_no_effect(driver)
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102147执行成功！")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102147_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102147",
            case_desc='102147 验证密码大于20个字符时，“完成”按钮不可点击',
            result=case_result,
