...
def _wait_resend_button(driver, timeout_s: int = 20):
    """
    等待 Resend 按钮出现（不要求可点击），返回元素。
    """
    # 不同版本可能是：
    # - Button name/label/value = "Resend"
    # - Button name/label/value 含倒计时，如 "Resend(59)" / "Resend in 59s"
    # - 甚至表现为 StaticText（不可点）+ Button 容器
    pred = (
...
def _debug_visible_texts(driver, limit: int = 30):
    try:
    ...
    for e in elems:
        try:
            if not e.is_displayed():
                continue
            t = (e.get_attribute("name") or e.get_attribute("label") or e.get_attribute("value") or "").strip()
            if t:
                texts.append(t)
        except Exception:
            continue
...
def _assert_resend_disabled(driver, timeout_s: int = 10):
    """
    验证“倒计时过程中 Resend 置灰且不可点击”：
    - 优先用 is_enabled()==False 判断
    - 若 enabled 属性不可靠，则尝试点击并断言不会触发成功点击（以不发生页面跳转为弱校验）
    """
    try:
    ...
    # 某些版本 enabled 可能恒为 True，这里用“点击无效”做兜底：尝试点击后短等 1s，仍在验证码页即可认为不可点击
    try:
        btn.click()
    except Exception:
        pass
    ...
    # 验证仍存在 Resend（或“重新发送”）按钮/文案（且依旧在当前页）
    try:
        _wait_resend_button(driver, timeout_s=3)
    except Exception:
        _debug_visible_texts(driver)
        raise AssertionError("点击后未再找到 Resend/重新发送，疑似已可点击或页面发生变化")
    return btn
...
def test_102136(setup_driver):
    """
    102136 验证验证码重发倒计时过程中“重新发送”，置灰且不可点击

    1. 重启 APP，确保未登录
    2. 进入 Forgot password，输入邮箱，Next 到验证码页
    3. 验证 Resend 在倒计时期间 disabled（置灰不可点）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        current_step = "步骤5: 验证倒计时期间Resend置灰不可点击"
        print(f"🔄 {current_step}")
        _assert_resend_disabled(driver, timeout_s=int(os.environ.get("RESEND_CHECK_TIMEOUT_S", "15")))
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102136执行成功！")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102136_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102136",
            case_desc='102136 验证验证码重发倒计时过程中“重新发送”，置灰且不可点击',
            result=case_result,
