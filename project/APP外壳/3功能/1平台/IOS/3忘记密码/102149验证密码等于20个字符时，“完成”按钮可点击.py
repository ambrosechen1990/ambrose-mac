...
def _assert_submit_enabled_and_success(driver, timeout_s: int = 25):
    """
    参考 102148：点击 Submit 后页面跳转，
    以“登录页协议文案出现”作为成功标志。
    """
...
def test_102149(setup_driver):
    """
    102149 验证密码等于20个字符时，“完成”按钮可点击
    ...
    password_value = os.environ.get("TEST_PASSWORD", 'Csx1234567Csx1234567')
    expectations = None  # 本用例不校验规则颜色
    ...
    try:
        ...
        current_step = "步骤7: 输入20位密码(两次)并收起键盘"
        print(f"🔄 {current_step}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102149_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102149",
            case_desc='102149 验证密码等于20个字符时，“完成”按钮可点击',
            result=case_result,
