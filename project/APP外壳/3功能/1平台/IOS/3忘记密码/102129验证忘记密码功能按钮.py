...
def test_102129(setup_driver):
    ...
    2. 点击登录按钮进入Sign in页面
    3. 点击忘记密码按钮
    4. 断言页面跳转，验证页面有"Forgot password"文本和"Next"按钮
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤4: 验证页面跳转，然后点击邮箱输入框并输入邮箱
        current_step = "步骤4: 验证页面跳转并输入邮箱"
        print(f"🔄 {current_step}")
        try:
            # 先验证页面已跳转到忘记密码页面
            forgot_password_text = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Forgot password"]'))
            )
            assert forgot_password_text.is_displayed(), "Forgot password文本存在但不可见"
            print(f"    ✅ 确认已跳转到忘记密码页面")
            ...
            assert email_value == email_address, \
                f"邮箱应该已输入，期望: {email_address}，实际: {email_value}"
            print(f"    ✅ 验证邮箱已输入: {email_value}")
        ...
        try:
            ...
            fail_reason = f"{current_step}失败: 无法找到或点击×按钮 - {str(e)}"
            print(f"❌ {fail_reason}")
            # 尝试查找所有可见的Button元素，用于调试
            try:
                all_buttons = driver.find_elements(AppiumBy.XPATH, '//XCUIElementTypeButton')
                visible_buttons = []
                for btn in all_buttons:
                    try:
                        if btn.is_displayed():
                            ...
                            if btn_name:
                                visible_buttons.append(btn_name)
                    except:
                        continue
                print(f"    💡 当前页面可见的按钮: {visible_buttons[:10]}")  # 显示前10个
            except:
                pass
            raise

        # 步骤6: 断言邮箱输入框内已输入的邮箱清空
        current_step = "步骤6: 断言邮箱输入框内已输入的邮箱清空"
        print(f"🔄 {current_step}")
        try:
            # 验证邮箱输入框内容已被清空
            email_input = WebDriverWait(driver, 10).until(
            ...
            # 检查邮箱输入框的值
            email_value_after = email_input.get_attribute("value")
            assert email_value_after == "" or email_value_after is None, \
                f"邮箱应该被清空，期望为空，实际: {email_value_after}"

            print(f"    ✅ 验证邮箱已被清空（值: {email_value_after}）")
            ...
            # 尝试获取当前邮箱输入框的值用于调试
            try:
                email_input = driver.find_element(AppiumBy.IOS_PREDICATE, 'type == "XCUIElementTypeTextField"')
                current_value = email_input.get_attribute("value")
                print(f"    💡 当前邮箱输入框的值: {current_value}")
            except:
                pass
            raise

        print("🎉 测试用例102129执行成功！")
        print("✅ 点击忘记密码按钮后能够正常跳转到忘记密码页面")
        print("✅ 邮箱输入和清空功能正常")
