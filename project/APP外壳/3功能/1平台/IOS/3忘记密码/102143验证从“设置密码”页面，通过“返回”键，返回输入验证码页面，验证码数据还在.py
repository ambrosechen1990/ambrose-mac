...
def _find_verification_input(driver, timeout: int = 12):
...
def _type_verification_code(driver, code: str, timeout: int = 12):
    target = _find_verification_input(driver, timeout=timeout)
    try:
        target.click()
    except Exception:
        rect = target.rect or {}
...
def _get_field_text(field) -> str:
    for attr in ("value", "name", "label"):
        try:
            v = (field.get_attribute(attr) or "").strip()
            if v:
                return v
        except Exception:
            continue
...
def _assert_verification_code_still_there(driver, expected_code: str, timeout_s: int = 12):
    """
    回到验证码页后，断言验证码输入框仍然有数据。
    iOS 密码/OTP 输入框有时会显示为圆点或格式化值，这里做弱校验：
    - value/name/label 任一非空
    - 且包含 expected_code，或长度>=len(expected_code)
    """

    def _ok(d):
        field = _find_verification_input(d, timeout=6)
        text = _get_field_text(field)
        if not text:
            return False
        if expected_code in text:
            return True
        # 可能显示为 "••••••" 或其他格式，至少保证“有输入”
        return len(text) >= len(expected_code)
...
def test_102143(setup_driver):
    """
    102143 验证从“设置密码”页面，通过“返回”键，返回输入验证码页面，验证码数据还在
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        current_step = "步骤6: 输入正确验证码并进入Set Password页"
        print(f"🔄 {current_step}")
        ...
        current_step = "步骤7: Set Password页点击返回(nav back)回到验证码页"
        print(f"🔄 {current_step}")
        _click_with_tap_fallback(
            driver,
            selectors=[
        ...
        current_step = "步骤8: 断言返回后验证码输入框数据仍在"
        print(f"🔄 {current_step}")
        _assert_verification_code_still_there(
            driver,
            expected_code=code,
            timeout_s=int(os.environ.get("CODE_STILL_THERE_TIMEOUT_S", "12")),
        )
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102143执行成功！")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102143_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102143",
            case_desc="102143 验证从“设置密码”页面，通过“返回”键，返回输入验证码页面，验证码数据还在",
            result=case_result,
