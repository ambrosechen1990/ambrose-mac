...
def test_102156(setup_driver):
    """
    102156 验证密码为数字+大+小写字母+字符时，“完成”按钮可点击
    ...
    password_value = os.environ.get("TEST_PASSWORD", 'XINGmai12?')
    ...
    try:
        ...
        current_step = "步骤7: 输入数字+大小写+特殊字符密码(两次)并收起键盘"
        print(f"🔄 {current_step}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102156_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102156",
            case_desc='102156 验证密码为数字+大+小写字母+字符时，“完成”按钮可点击',
            result=case_result,
