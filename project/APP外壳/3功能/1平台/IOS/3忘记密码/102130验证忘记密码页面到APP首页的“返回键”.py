...
def test_102130(setup_driver):
    ...
    3. 点击忘记密码按钮
    4. 点击左上角返回按钮
    5. 断言页面跳转至Sign in页面，验证页面显示用户协议和隐私政策文本
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤3: 点击忘记密码按钮
        current_step = "步骤3: 点击忘记密码按钮"
        print(f"🔄 {current_step}")
        try:
            forgot_password_btn = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="Forgot password"]'))
            )
            forgot_password_btn.click()
            print(f"✅ {current_step} - 完成")
            time.sleep(2)  # 等待页面跳转
        except Exception as e:
            fail_reason = f"{current_step}失败: 无法找到或点击忘记密码按钮 - {str(e)}"
            print(f"❌ {fail_reason}")
            ...
            try:
                ...
                for txt in all_texts:
                    try:
                        if txt.is_displayed():
                            text_name = txt.get_attribute("name") or ""
                            if text_name:
                                visible_texts.append(text_name)
                    except:
                        continue
                print(f"    💡 当前页面可见的StaticText元素: {visible_texts[:10]}")  # 显示前10个
            except:
                pass
            raise
        ...
        try:
            ...
            try:
                ...
                generic_back_buttons = driver.find_elements(AppiumBy.XPATH,
                ...
                print(f"    ⚠️ 通用返回方法也失败: {e2}")
                raise

        # 步骤5: 断言页面跳转至Sign in页面，验证页面显示用户协议和隐私政策文本
        current_step = "步骤5: 断言页面跳转至Sign in页面，验证用户协议和隐私政策文本"
        print(f"🔄 {current_step}")
        try:
            ...
            assert agreement_text.is_displayed(), "用户协议和隐私政策文本存在但不可见"
            agreement_text_value = agreement_text.get_attribute("name")
            expected_text = "I have read and understood the Privacy Policy and agree to the User Agreement."
            assert agreement_text_value == expected_text, \
                f"用户协议和隐私政策文本内容不正确，期望'{expected_text}'，实际: {agreement_text_value}"
            print(f"    ✅ 找到用户协议和隐私政策文本: {agreement_text_value}")

            print(f"✅ {current_step} - 完成，已成功返回Sign in页面")
            print(f"✅ 页面包含以下关键元素：")
            print(f"   - 用户协议和隐私政策文本: {agreement_text_value}")
        except Exception as e:
            fail_reason = f"{current_step}失败: 未找到用户协议和隐私政策文本，可能未成功返回Sign in页面 - {str(e)}"
            print(f"❌ {fail_reason}")
            # 尝试查找所有可见的TextView元素，用于调试
            try:
                all_textviews = driver.find_elements(AppiumBy.XPATH, '//XCUIElementTypeTextView')
                visible_textviews = []
                for tv in all_textviews:
                    try:
                        if tv.is_displayed():
                            text_name = tv.get_attribute("name") or ""
                            if text_name:
                                visible_textviews.append(text_name)
                    except:
                        continue
                print(f"    💡 当前页面可见的TextView元素: {visible_textviews[:10]}")  # 显示前10个

                # 也尝试查找StaticText元素
                all_static_texts = driver.find_elements(AppiumBy.XPATH, '//XCUIElementTypeStaticText')
                visible_static_texts = []
                for st in all_static_texts:
                    try:
                        if st.is_displayed():
                            text_name = st.get_attribute("name") or ""
                            if text_name:
                                visible_static_texts.append(text_name)
                    except:
                        continue
                print(f"    💡 当前页面可见的StaticText元素: {visible_static_texts[:10]}")  # 显示前10个
            except:
                pass
            raise

        print("🎉 测试用例102130执行成功！")
        print("✅ 点击返回按钮后能够正常返回Sign in页面")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102130_failed")
