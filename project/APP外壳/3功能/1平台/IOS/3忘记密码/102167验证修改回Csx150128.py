...
def _back_to_beatbot_after_gmail(
    driver, *, beatbot_bundle_id: str, gmail_bundle_id: str = "com.google.Gmail"
) -> None:
    """
    Gmail 取码后，经常仍停留在 noreply 详情页（前台还是 Gmail）。
    `gmail_otp_utils.get_gmail_verification_code` 只负责返回验证码，最多 terminate Gmail，
    但 terminate 不一定让系统自动切回被测 APP，所以这里强制切回并校验页面已回到验证码输入页。
    """
    # 尽力关闭 Gmail（即使 get_gmail_verification_code 已做过，这里再补一刀）
    try:
    ...
    for _ in range(4):
        try:
            ...
            time.sleep(1.2)
            _assert_on_verification_page(driver, timeout=10)
            return
        except Exception as ex:
            last_err = ex
            time.sleep(1.0)
            continue
    raise RuntimeError(f"切回被测APP失败（仍可能停留在Gmail）：{last_err}")
...
def _find_submit_button(driver, timeout_s: int = 12):
    ...
    if not e.is_displayed():
        raise TimeoutException("Submit按钮存在但不可见")
    return e


def _assert_submit_disabled_or_no_effect(driver):
    """
    断言密码<6位时 Submit 不可点击：
    - 优先用 is_enabled() == False
    - 若 enabled 不可靠，则点击后断言仍停留 Set Password 页
    """
...
def _assert_password_rules_colors_like_102224(driver, timeout_s: int = 10):
    """
    参考 102224：密码<6位时，断言规则文案存在；若可获取颜色则校验：
    - • 6-20 characters 期望红色
    - 其余三条期望灰色
    注：iOS 原生元素通常无法稳定获取 color，这里以“拿到就校验，拿不到就记录”为准。
    """
    rule_expectations = {
        '• 6-20 characters': "red",
        '• contains letters': "gray",
        '• contains numbers': "gray",
        '• Supports special characters:! @ # $ % ^ & * ( ) - _ = + \\ | [ ] { } ; : / ? . , ~ > < `': "gray",
    }

    for rule_text, expect_color in rule_expectations.items():
        sel = f'//XCUIElementTypeStaticText[@name="{rule_text}"]'
        rule_elem = WebDriverWait(driver, timeout_s).until(
        ...
        cv = str(color_val).lower()
        if expect_color == "red":
            assert ("255" in cv or "#ff" in cv or "red" in cv), f"期望红色但实际为 {color_val}"
        else:
            assert (
                "128" in cv
                or "gray" in cv
...
def _assert_submit_enabled_and_success_by_agreement_text(driver, timeout_s: int = 25) -> None:
    """
    点击 Submit 后页面跳转，断言出现登录页协议文案即成功：
    ...
    try:
        ...
        enabled = True
    assert enabled is True, "Submit按钮不可点击（is_enabled=False）"
...
def test_102148(setup_driver):
    """
    102148 验证密码等于6个字符时，“完成”(Submit)按钮可点击（忘记密码链路）

    步骤:
    1. 重启APP并处理登录状态，停留在登录落地页
    2. 点击 Sign In 进入登录页
    3. 点击 Forgot password 进入忘记密码页
    4. 输入邮箱并点击 Next 进入验证码页
    5. Gmail 获取最新验证码
    6. 输入验证码进入 Set Password 页
    7. 在 Password / Retype Password 输入 6 位密码（例如 Csx15028）
    8. 点击 Done 收起键盘，点击 Submit，断言跳转到登录页并显示协议文案 TextView
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    pwd_6 = os.environ.get("PASSWORD_6", "Csx150128")  # == 6
    ...
    try:
        ...
        code = get_gmail_verification_code(
            ...
            timeout_s=int(os.environ.get("GMAIL_TIMEOUT_S", "30")),
            gmail_bundle_id=gmail_bundle_id,
            kill_gmail_after=True,
        )
        _back_to_beatbot_after_gmail(
            driver, beatbot_bundle_id=beatbot_bundle_id, gmail_bundle_id=gmail_bundle_id
        )
        time.sleep(1.0)
        ...
        try:
            ...
            pass
        pwd2.send_keys(pwd_6)
        time.sleep(0.6)
        print(f"✅ {current_step} - 完成（pwd_6: {pwd_6}）")

        current_step = "步骤8: Done收起键盘，点击Submit跳转并断言协议文案"
        print(f"🔄 {current_step}")
        _dismiss_keyboard(driver)
        _assert_submit_enabled_and_success_by_agreement_text(
            driver, timeout_s=int(os.environ.get("SUBMIT_SUCCESS_TIMEOUT_S", "25"))
        )
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102148执行成功！")
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102148_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102148",
            case_desc='102148 验证密码等于6个字符时，“完成”按钮可点击',
            result=case_result,
