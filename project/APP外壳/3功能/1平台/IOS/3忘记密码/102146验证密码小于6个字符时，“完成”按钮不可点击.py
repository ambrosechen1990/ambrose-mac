...
def test_102146(setup_driver):
    """
    102146 验证密码小于6个字符时，“完成”(Submit)按钮不可点击（忘记密码链路）
    ...
    7. 在 Password / Retype Password 输入 <6 位密码（例如 Csx15）
    8. 断言四条规则文案存在；若可获取颜色则校验：• 6-20 characters 为红，其余为灰
    9. 点击 Done 收起键盘，点击 Submit，断言仍停留在 Set Password 页（按钮不可点击/点击无效）
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    short_pwd = os.environ.get("SHORT_PASSWORD", "Csx15")  # < 6
    ...
    try:
        ...
        try:
            ...
            pass
        pwd2.send_keys(short_pwd)
        time.sleep(0.6)
        print(f"✅ {current_step} - 完成（short_pwd: {short_pwd}）")

        current_step = "步骤8: 断言密码规则提示/颜色（参考102224）"
        print(f"🔄 {current_step}")
        _assert_password_rules_colors_like_102224(driver, timeout_s=10)
        print(f"✅ {current_step} - 完成")

        current_step = "步骤9: Done收起键盘，点击Submit仍停留Set Password页"
        print(f"🔄 {current_step}")
        _dismiss_keyboard(driver)
        _assert_submit_disabled_or_no_effect(driver)
        print(f"✅ {current_step} - 完成")

        print("🎉 测试用例102146执行成功！")
        time.sleep(2)
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102146_failed")
        ...
        write_report(
            ...
            platform="ios",
            case_id="102146",
            case_desc='102146 验证密码小于6个字符时，“完成”按钮不可点击',
            result=case_result,
