...
def _assert_password_recovery_failed_dialog(driver, timeout_s: int = 20):
    """验证码过期后提交密码，应弹出「Password recovery failed…」提示。"""
    xpath = (
        '//XCUIElementTypeStaticText[@name="Password recovery failed. Please try again."]'
    )
    el = WebDriverWait(driver, timeout_s).until(
        EC.presence_of_element_located((AppiumBy.XPATH, xpath))
    )
    assert el.is_displayed(), "未显示 Password recovery failed 弹框文案"
...
def test_102166(setup_driver):
    ...
    try:
        ...
        print(f"🔄 {current_step}")
        time.sleep(wait_s)
        current_step = "步骤8: 密码与确认密码输入 Csx150128，Submit 后断言失败弹框"
        print(f"🔄 {current_step}")
        ...
        _assert_password_recovery_failed_dialog(
            driver, timeout_s=int(os.environ.get("RECOVERY_FAILED_ALERT_TIMEOUT_S", "20"))
        )
