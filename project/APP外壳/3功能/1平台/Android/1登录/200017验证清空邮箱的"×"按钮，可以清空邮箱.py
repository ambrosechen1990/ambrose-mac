...
def test_200017(setup_driver):
    ...
    try:
        ...
        current_step = "步骤5: 点击邮箱清空按钮"
        print(f"🔄 {current_step}")
        try:
            print(f"    🔄 定位: {XPATH_CLEAR_EMAIL}")
            clear_btn = WebDriverWait(driver, 18).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, XPATH_CLEAR_EMAIL))
            )
            clear_btn.click()
            time.sleep(0.5)
            email_el = driver.find_element(AppiumBy.XPATH, XPATH_EMAIL)
            assert not (email_el.text or "").strip(), "邮箱未清空"
            print(f"✅ {current_step} - 完成")
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200017_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200017",
            case_desc='200017 验证清空邮箱的×按钮，可以清空邮箱',
            result=case_result,
