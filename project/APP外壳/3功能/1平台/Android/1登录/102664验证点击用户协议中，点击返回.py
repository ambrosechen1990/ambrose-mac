...
def test_102664(setup_driver):
    """
    验证点击用户协议中，点击返回（Android）

    1. 重置APP，检测登录状态，如果已登录则登出
    2. 点击登录按钮进入Sign in页面
    3. 找到协议文本，点击User Agreement超链接
    4. 验证页面跳转到用户协议页面，检查WebView
    5. 点击左上角返回按钮
    6. 断言页面返回到登录页面，显示协议文本
    """
    driver = setup_driver
    case_result = "success"
    fail_reason = ""
    ...
    try:
        ...
        # 步骤4: 验证页面跳转到用户协议页面，检查WebView
        current_step = "步骤4: 验证用户协议页面，检查WebView"
        print(f"🔄 {current_step}")
        try:
            # 验证页面有WebView
        ...
        try:
            ...
            # 如果指定的XPath找不到，尝试其他方式
            print(f"    ⚠️ 使用指定XPath未找到返回按钮，尝试其他方式: {e}")
            try:
                # 方式1: 尝试使用系统返回键
                ...
                fail_reason = f"{current_step}失败: 无法找到或点击返回按钮 - {e}; 系统返回键也失败: {e2}"
                print(f"❌ {fail_reason}")
                raise

        # 步骤6: 断言页面返回到登录页面，显示协议文本
        current_step = "步骤6: 断言页面返回到登录页面，显示协议文本"
        print(f"🔄 {current_step}")
        try:
            # 验证页面有指定的协议文本TextView
            expected_text = "I have read and understood the Privacy Policy and agree to the User Agreement."
            agreement_text_view = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located(
                    (AppiumBy.XPATH, f'//android.widget.TextView[@text="{expected_text}"]')
                )
            )
            assert agreement_text_view.is_displayed(), "协议文本TextView存在但不可见"
            actual_text = agreement_text_view.get_attribute("text") or ""
            print(f"    ✅ 找到协议文本TextView: {actual_text}")
            
            # 验证文本内容匹配
            assert expected_text == actual_text, f"协议文本不匹配，期望: {expected_text}，实际: {actual_text}"
            print(f"    ✅ 协议文本内容正确")
            
            # 额外验证：确认在Sign in页面（可以检查是否有登录相关的元素）
            try:
                # 检查是否有邮箱或密码输入框
                ...
                if email_inputs:
                    print(f"    ✅ 确认已返回Sign in页面（找到邮箱输入框）")
            except:
                print(f"    ℹ️ 无法确认Sign in页面，但协议文本已找到")
            
            print(f"✅ {current_step} - 完成，用例执行成功")
            print("🎉 测试用例102664执行成功！")
        except Exception as e:
            fail_reason = f"{current_step}失败: 未找到指定的协议文本TextView或文本不匹配 - {e}"
            print(f"❌ {fail_reason}")
        ...
        print("=" * 60)
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102664_failed", RUN_DIR)
