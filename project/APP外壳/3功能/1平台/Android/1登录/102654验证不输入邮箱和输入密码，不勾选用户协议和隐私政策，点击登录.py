...
def test_102654(setup_driver):
    """
    验证不输入邮箱和输入密码，不勾选用户协议和隐私政策，点击登录（Android）
    ...
    try:
        ...
        try:
            # 验证邮箱输入框为空
            email_input_xpath = "//androidx.compose.ui.platform.ComposeView/android.view.View/android.view.View/android.view.View/android.widget.EditText[1]"
            email_inputs = driver.find_elements(AppiumBy.XPATH, email_input_xpath)
            if email_inputs:
                email_value = email_inputs[0].get_attribute("text") or ""
                if email_value:
                    # 如果邮箱输入框有内容，清空它
                    try:
                        ...
                        print(f"    ⚠️ 清空邮箱输入框失败，但继续执行")
                else:
                    print(f"    ✅ 邮箱输入框为空")
            else:
                print(f"    ℹ️ 未找到邮箱输入框，可能页面还未完全加载")
        ...
        # 步骤6: 不勾选用户协议和隐私政策（跳过此步骤）
        current_step = "步骤6: 不勾选用户协议和隐私政策"
        print(f"🔄 {current_step}")
        print("    ℹ️ 保持协议勾选框未勾选状态")
        time.sleep(0.5)
        ...
        try:
            ...
            error_text = error_message.get_attribute("text") or ""
        ...
        print("=" * 60)
        traceback.print_exc()
        save_failure_screenshot(driver, "test_102654_failed", RUN_DIR)
