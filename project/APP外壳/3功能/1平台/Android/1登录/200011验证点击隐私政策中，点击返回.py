"""
200011 验证点击隐私政策中，点击返回（Android）。

说明：UiAutomator2 通常不把 TextView 内 ClickableSpan（Privacy Policy / User Agreement）拆成独立节点，
无法用单独 xpath 表示「超链接」。本脚本：先找可点击短文案或与整句 bounds 相交的可点击节点；再在整句矩形内
按字符串位置与多行 Y 做 clickGesture/tap，并用页面特征确认已进入隐私页。返回优先点左上角 Navigate up/back。
整控件 click() 往往点到整段文本中间，无法触发 span；因此需坐标或子节点策略（见模块内函数注释）。

步骤对齐 iOS 102661：重启 → 登出如需要 → Sign In → Privacy Policy → 左上角返回 → 断言协议整句仍在。
"""
...
# 返回按钮备用
XPATH_BACK_ALT = '//*[@content-desc="back"]'
...
def test_200011(setup_driver):
    ...
    try:
        ...
        # 步骤4: 点击 Privacy Policy 链接
        current_step = "步骤4: 点击 Privacy Policy 链接"
        print(f"🔄 {current_step}")
        try:
            print("    🔄 _tap_privacy_policy_on_sign_in_disclaimer")
        ...
        # 步骤5: 隐私政策页点击返回
        current_step = "步骤5: 隐私政策页点击返回"
        print(f"🔄 {current_step}")
        try:
            clicked = False
            for xp in (XPATH_BACK, XPATH_BACK_ALT):
                try:
        ...
        # 步骤6: 断言回到 Sign In 页并显示协议整句
        current_step = "步骤6: 断言回到 Sign In 页并显示协议整句"
        print(f"🔄 {current_step}")
        try:
            print(f"    🔄 定位: {XPATH_DISCLAIMER_ASSERT}")
            el = WebDriverWait(driver, 18).until(
                EC.presence_of_element_located((AppiumBy.XPATH, XPATH_DISCLAIMER_ASSERT))
            )
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200011_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200011",
            case_desc="200011 验证点击隐私政策中，点击返回",
            result=case_result,
