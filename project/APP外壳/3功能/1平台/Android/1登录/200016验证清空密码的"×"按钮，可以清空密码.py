...
# 密码右侧清空 ×
XPATH_CLEAR_PASSWORD = '//android.widget.ImageView[@content-desc="clear"]'
...
def test_200016(setup_driver):
    ...
    try:
        ...
        current_step = "步骤5: 点击密码清空按钮"
        print(f"🔄 {current_step}")
        try:
            print(f"    🔄 定位: {XPATH_CLEAR_PASSWORD}")
            clear_btn = WebDriverWait(driver, 18).until(
                EC.element_to_be_clickable((AppiumBy.XPATH, XPATH_CLEAR_PASSWORD))
            )
            clear_btn.click()
            time.sleep(0.5)
            pwd_el = driver.find_element(AppiumBy.XPATH, XPATH_PASSWORD)
            assert not (pwd_el.text or "").strip(), "密码未清空"
            print(f"✅ {current_step} - 完成")
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200016_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200016",
            case_desc='200016 验证清空密码的×按钮，可以清空密码',
            result=case_result,
