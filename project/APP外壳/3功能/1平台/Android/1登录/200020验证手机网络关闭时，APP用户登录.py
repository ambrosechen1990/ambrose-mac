"""
200020 验证手机网络关闭时，APP用户登录（Android）。

关闭网络后登录，断言 Network connection error；finally 恢复网络。
"""
...
# 本用例期望的提示/断言文案
MSG_NETWORK_ERROR = "Network connection error, please check"
...
# ---------- 网络开关（adb / mobile:shell，200020 专用）----------

def _shell(driver, cmd: str):
    parts = cmd.strip().split()
    return driver.execute_script("mobile: shell", {"command": parts[0], "args": parts[1:]})


def _adb_shell(cmd: str):
    serial = (os.environ.get("ANDROID_UDID") or os.environ.get("ANDROID_SERIAL") or "").strip()
    base = ["adb"] + (["-s", serial] if serial else [])
    subprocess.run(
        base + ["shell"] + cmd.strip().split(),
        check=True,
...
def _shell_with_fallback(driver, cmd: str):
    try:
        return _shell(driver, cmd)
    except Exception as e:
        if "adb_shell" in str(e) and "has not been enabled" in str(e):
            print(f"    ⚠️ mobile:shell 禁用，改用 adb: {cmd}")
            return _adb_shell(cmd)
        raise


def _set_network(driver, enabled: bool):
    if enabled:
        _shell_with_fallback(driver, "svc wifi enable")
        _shell_with_fallback(driver, "svc data enable")
    else:
        _shell_with_fallback(driver, "svc wifi disable")
        _shell_with_fallback(driver, "svc data disable")
    time.sleep(1.5)


def test_200020(setup_driver):
    ...
    try:
        ...
        current_step = "步骤4: 关闭手机网络（wifi+data）"
        print(f"🔄 {current_step}")
        try:
            _set_network(driver, False)
        ...
        current_step = "步骤8: 勾选用户协议与隐私政策"
        print(f"🔄 {current_step}")
        try:
            print(f"    🔄 定位: {XPATH_CHECKBOX}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200020_failed")
        assert False, f"测试失败 - {fail_reason}"
    finally:
        # 200020：恢复网络，避免影响后续用例
        if network_disabled:
            try:
                print("    🔄 finally: 恢复网络")
                _set_network(driver, True)
            except Exception as ex:
                print(f"    ⚠️ finally 恢复网络失败: {ex}")
        ...
        write_report(
            ...
            platform="android",
            case_id="200020",
            case_desc="200020 验证手机网络关闭时，APP用户登录",
            result=case_result,
