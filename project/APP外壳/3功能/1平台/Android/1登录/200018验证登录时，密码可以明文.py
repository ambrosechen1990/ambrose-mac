...
def _password_field_text_blob(el) -> str:
    """汇总密码框可见文案（Compose 下 password 属性不可靠，以 text 为准）。"""
    parts: list[str] = []
...
def _assert_password_plain_visible(driver, pwd_xpath: str, expected: str, timeout_s: float = 8.0) -> str:
    """
    断言密码已切换为明文。
    Android Compose 的 password 属性常为 true 且不随可见性变化，故对齐 iOS：检查可读文本含预期密码。
    """
    ...
    while time.time() < deadline:
        ...
        for sub in pwd_el.find_elements(AppiumBy.XPATH, ".//*[@text]"):
            try:
                ...
                continue
        last_state = (
            f"blob={blob!r}, password_attr={pwd_el.get_attribute('password')!r}, "
            f"inputType={pwd_el.get_attribute('inputType')!r}"
        )
        time.sleep(0.35)
    raise AssertionError(f"密码未显示为明文（期望包含 {expected!r}），最后状态: {last_state}")


def _click_password_visibility_toggle(driver, timeout_s: int = 18):
    """点击 lock 切换明文；click 失败时用坐标点击。"""
...
def test_200018(setup_driver):
    ...
    try:
        ...
        current_step = "步骤5: 点击密码可见性按钮并断言明文"
        print(f"🔄 {current_step}")
        try:
            print(f"    🔄 定位并点击: {XPATH_PASSWORD_EYE}")
            _click_password_visibility_toggle(driver)
            try:
                visible_blob = _assert_password_plain_visible(driver, XPATH_PASSWORD, pwd_value)
            except AssertionError:
                print("    ℹ️ 首次点击后未读到明文，重试点击 lock")
                _click_password_visibility_toggle(driver)
                visible_blob = _assert_password_plain_visible(driver, XPATH_PASSWORD, pwd_value)
            print(f"    ✅ 密码已明文显示: {visible_blob[:32]!r}...")
            print(f"✅ {current_step} - 完成")
        except Exception as e:
            fail_reason = f"{current_step}失败: {str(e)}"
            print(f"❌ {fail_reason}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200018_failed")
