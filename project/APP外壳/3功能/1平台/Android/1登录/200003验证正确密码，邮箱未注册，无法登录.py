"""
200003 验证正确密码，邮箱未注册，无法登录（Android）。

对齐 iOS 102652：邮箱未注册提示。
"""
...
# 本用例期望的提示/断言文案
MSG_NOT_REGISTERED = "This email is not registered. Please check and re-enter."
...
def test_200003(setup_driver):
    ...
    try:
        ...
        print(f"🔄 {current_step}")
        try:
            email_value = os.environ.get("UNREGISTERED_EMAIL", "haoc51888chenhao@gmail.com")
        ...
        # 步骤7：勾选免责声明 checkbox 后才能提交
        current_step = "步骤7: 勾选用户协议与隐私政策"
        print(f"🔄 {current_step}")
        try:
        ...
        # 步骤9：等待 Toast/页面上的错误文案
        current_step = "步骤9: 断言提示文案"
        print(f"🔄 {current_step}")
        try:
            msg = MSG_NOT_REGISTERED
            # 兼容完整句或前缀匹配
            xpath_assert = (
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200003_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200003",
            case_desc="200003 验证正确密码，邮箱未注册，无法登录",
            result=case_result,
