"""
200013 验证点击用户协议中，点击返回（Android）。

步骤：重启 → 登出/入口页 → Sign In → User Agreement → 返回 → 断言免责声明。
"""
...
# Sign In 免责声明整句（Privacy Policy / User Agreement 为 ClickableSpan，无独立节点）
...
# ---------- 免责声明内 User Agreement 点击（ClickableSpan 无独立节点，需坐标/短节点策略）----------
...
def _user_agreement_page_indicators_hit(driver) -> bool:
    """
    是否已进入用户协议页（对齐 200011 隐私页判定：WebView/返回栏 + 协议正文特征）。
    仍停在 Sign In 且免责声明可见时返回 False。
    """
    ...
    if _any_visible('//*[contains(@text,"Last Modified")]'):
        return True
    if _any_visible('//*[contains(@text,"Introduction") and string-length(@text) < 64]'):
        return True
...
def _tap_user_agreement_on_sign_in_disclaimer(driver, timeout_s: int = 18) -> None:
    ...
    wrapped = h > 52
    if wrapped:
        # 换行时「User Agreement」多在第二行偏左，不能只用整句字符比例（~0.82）点第一行右侧
...
def test_200013(setup_driver):
    ...
    try:
        ...
        current_step = "步骤4: 点击 User Agreement（免责声明内超链接，非整句 TextView）"
        print(f"🔄 {current_step}")
        try:
        ...
        current_step = "步骤6: 断言免责声明整句"
        print(f"🔄 {current_step}")
        try:
            xp = (
                f'//android.widget.TextView[@text="{DISCLAIMER_TEXT}"]'
                ' | //android.widget.TextView[contains(@text,"User Agreement") '
                'and contains(@text,"Privacy Policy") and string-length(@text) < 220]'
            )
            el = WebDriverWait(driver, 18).until(EC.presence_of_element_located((AppiumBy.XPATH, xp)))
        ...
        if not fail_reason:
            fail_reason = f"{current_step}失败: {str(e)}"
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200013_failed")
        assert False, fail_reason
    finally:
        # 无论成功失败都写入 platform 报告
        write_report(
            run_dir=RUN_DIR, run_label=RUN_LABEL, run_ts=RUN_TS,
            platform="android", case_id="200013",
            case_desc="200013 验证点击用户协议中，点击返回",
            result=case_result, fail_reason=fail_reason,
        )
