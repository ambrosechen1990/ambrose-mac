"""
200015 验证正确账号+密码（Android）。

对齐 iOS 102667：登录成功后出现 More。
"""
...
def test_200015(setup_driver):
    ...
    try:
        ...
        current_step = "步骤9: 断言登录成功（More 可见）"
        print(f"🔄 {current_step}")
        try:
            timeout_s = int(os.environ.get("LOGIN_SUCCESS_TIMEOUT_S", "45"))
            print(f"    🔄 定位: {XPATH_LOGGED_IN_MORE}")
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200015_failed")
