"""
200007 验证输入邮箱，不输入密码，不勾选用户协议和隐私政策，点击登录（Android）。

仅邮箱 + Next（不勾选）→ Incorrect password.
"""
...
# 本用例期望的提示/断言文案
MSG_INCORRECT_PASSWORD = "Incorrect password. Please confirm and try again."
...
def test_200007(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200007_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200007",
            case_desc="200007 验证输入邮箱，不输入密码，不勾选用户协议和隐私政策，点击登录",
            result=case_result,
