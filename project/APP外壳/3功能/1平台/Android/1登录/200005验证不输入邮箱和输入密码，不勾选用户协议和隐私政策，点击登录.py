"""
200005 验证不输入邮箱和输入密码，不勾选用户协议和隐私政策，点击登录（Android）。

仅输入密码 + Next（不勾选）→ Please enter a valid email address.
"""
...
def test_200005(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200005_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200005",
            case_desc="200005 验证不输入邮箱和输入密码，不勾选用户协议和隐私政策，点击登录",
            result=case_result,
