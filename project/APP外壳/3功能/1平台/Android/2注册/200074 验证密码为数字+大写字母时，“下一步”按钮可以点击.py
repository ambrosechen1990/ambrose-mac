"""
200074 验证密码为数字+大写字母时，下一步按钮可以点击（Android）。

  步骤：进入密码页验证（验证密码为数字+大写字母时，下一步按钮可以点击）。
"""
...
def test_200074(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200074_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200074",
            case_desc='200074 验证密码为数字+大写字母时，下一步按钮可以点击',
            result=case_result,
