"""
200079 验证密码为大写字母+字符时，下一步按钮不可以点击（Android）。

  步骤：进入密码页验证（验证密码为大写字母+字符时，下一步按钮不可以点击）。
"""
...
def test_200079(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200079_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200079",
            case_desc='200079 验证密码为大写字母+字符时，下一步按钮不可以点击',
            result=case_result,
