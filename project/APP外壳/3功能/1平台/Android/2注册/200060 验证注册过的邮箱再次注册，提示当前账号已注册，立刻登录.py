"""
200060 验证注册过的邮箱再次注册，提示当前账号已注册，立刻登录（Android）。

  步骤1～3：重启 → 入口页 → Sign Up
  步骤4：邮箱栏输入已注册邮箱 haoc51888@gmail.com
  步骤5：勾选隐私政策
  步骤6：点击 Next
  步骤7：断言弹出「账号已注册」提示（含 Confirm / Cancel）
"""
...
def test_200060(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200060_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200060",
            case_desc='200060 验证注册过的邮箱再次注册，提示当前账号已注册，立刻登录',
            result=case_result,
