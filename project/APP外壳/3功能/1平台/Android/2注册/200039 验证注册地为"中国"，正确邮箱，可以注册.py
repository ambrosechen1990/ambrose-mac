"""
200039 验证注册地为"中国"，正确邮箱，可以注册（Android）。

  步骤：国家框输入 china → 收起键盘 → 点击 China → 输入邮箱 → 勾选 → Next 进入密码页。
"""
...
from register_case_base import (  # noqa: E402
    ...
    step_ensure_back_on_signup_page,
...
def test_200039(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200039_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200039",
            case_desc='200039 验证注册地为"中国"，正确邮箱，可以注册',
            result=case_result,
