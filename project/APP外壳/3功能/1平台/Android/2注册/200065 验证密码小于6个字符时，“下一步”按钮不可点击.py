"""
200065 验证密码小于6个字符时，下一步按钮不可点击（Android）。

  步骤：进入密码页验证（验证密码小于6个字符时，下一步按钮不可点击）。
"""
...
def test_200065(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200065_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200065",
            case_desc='200065 验证密码小于6个字符时，下一步按钮不可点击',
            result=case_result,
