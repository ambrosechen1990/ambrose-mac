"""
200078 验证密码为小写字母+字符时，下一步按钮不可以点击（Android）。

  步骤：进入密码页验证（验证密码为小写字母+字符时，下一步按钮不可以点击）。
"""
...
def test_200078(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200078_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200078",
            case_desc='200078 验证密码为小写字母+字符时，下一步按钮不可以点击',
            result=case_result,
