"""
200063 验证进入设置密码页面（Android）。

  步骤：进入密码页验证（验证进入设置密码页面）。
"""
...
def test_200063(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200063_failed")
