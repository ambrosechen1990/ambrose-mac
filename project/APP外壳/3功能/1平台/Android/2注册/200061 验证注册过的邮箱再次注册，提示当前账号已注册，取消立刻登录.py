"""
200061 验证注册过的邮箱再次注册，提示当前账号已注册，取消立刻登录（Android）。

  步骤1～3：重启 → 入口页 → Sign Up
  步骤4：邮箱栏输入已注册邮箱 haoc51888@gmail.com
  步骤5：勾选隐私政策
  步骤6：点击 Next
  步骤7：断言弹出「账号已注册」提示
  步骤8：点击 Cancel
...
def _assert_already_registered_alert(driver, timeout_s: int = 15) -> None:
    """断言已注册提示弹框及 Confirm / Cancel 按钮可见。"""
    alert = None
...
def _click_alert_cancel(driver, timeout_s: int = 10) -> None:
    cancel_btn = WebDriverWait(driver, timeout_s).until(
        EC.element_to_be_clickable((AppiumBy.XPATH, XPATH_ALERT_CANCEL))
    )
    cancel_btn.click()
    time.sleep(1.0)


def _assert_alert_dismissed(driver, timeout_s: int = 8) -> None:
    """Cancel 后弹框应关闭。"""
    ...
    while time.time() < deadline:
        ...
        if not visible:
            return
        time.sleep(0.4)
    raise TimeoutException("点击 Cancel 后已注册提示弹框仍未关闭")


def _xpath_email_by_value(email: str) -> str:
    return f'//android.widget.EditText[@text="{email}"]'


def _assert_still_on_signup_after_cancel(driver, email: str, timeout_s: int = 12) -> None:
    """
    Cancel 后仍应在注册主页面。
    注意：Cancel 后邮箱框可能不再挂在 ScrollView 下，故不用单一 XPATH_EMAIL 强校验。
    """
    WebDriverWait(driver, timeout_s).until(
        EC.presence_of_element_located((AppiumBy.XPATH, XPATH_CHECKBOX))
    )
    next_btn = WebDriverWait(driver, timeout_s).until(
        EC.presence_of_element_located((AppiumBy.XPATH, XPATH_NEXT))
    )
    assert next_btn.is_displayed(), "取消后应仍在注册页（Next 按钮不可见）"

    # 不应进入设置密码页（通常会出现第 2 个密码框或 lock 图标）
    ...
    if len(pwd_fields) >= 2 or lock_icons:
        raise AssertionError("取消后不应进入设置密码页")

    # 邮箱：多种结构兜底（输入后常为 @text=邮箱，不一定在 ScrollView 内）
    email_xpaths = (
        _xpath_email_by_value(email),
        XPATH_EMAIL,
        XPATH_EMAIL_FALLBACK,
    ...
    for xp in email_xpaths:
        try:
            for el in driver.find_elements(AppiumBy.XPATH, xp):
                ...
                if shown:
                    print(f"    📝 注册页邮箱框内容: {shown!r}（定位: {xp}）")
                    return
        except Exception:
            continue
    print("    ℹ️ 取消后未读到邮箱文案，但 checkbox + Next 仍在，判定仍在注册页")
...
def test_200061(setup_driver):
    ...
    try:
        ...
        current_step = "步骤8: 点击 Cancel"
        print(f"🔄 {current_step}")
        try:
            _click_alert_cancel(driver)
            _assert_alert_dismissed(driver)
            print(f"✅ {current_step} - 完成")
        except Exception as e:
            fail_reason = f"{current_step}失败: {e}"
            raise

        current_step = "步骤9: 断言仍停留在注册页"
        print(f"🔄 {current_step}")
        try:
            _assert_still_on_signup_after_cancel(driver, email, timeout_s=12)
            print(f"✅ {current_step} - 完成")
        except Exception as e:
            fail_reason = f"{current_step}失败: {e}"
            raise

        print("🎉 测试用例 200061 执行成功！")
    except Exception as e:
        case_result = "failed"
        if not fail_reason:
            fail_reason = f"{current_step}失败: {e}"
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200061_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200061",
            case_desc='200061 验证注册过的邮箱再次注册，提示当前账号已注册，取消立刻登录',
            result=case_result,
