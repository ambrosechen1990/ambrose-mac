"""
200069 验证密码为纯数字时，下一步按钮不可点击（Android）。

  步骤：进入密码页验证（验证密码为纯数字时，下一步按钮不可点击）。
"""
...
def test_200069(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200069_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200069",
            case_desc='200069 验证密码为纯数字时，下一步按钮不可点击',
            result=case_result,
