"""
200072 验证密码为纯字符时，下一步按钮不可点击（Android）。

  步骤：进入密码页验证（验证密码为纯字符时，下一步按钮不可点击）。
"""
...
def test_200072(setup_driver):
    ...
    try:
        ...
        print(f"{'=' * 60}")
        traceback.print_exc()
        save_failure_screenshot(driver, "test_200072_failed")
        ...
        write_report(
            ...
            platform="android",
            case_id="200072",
            case_desc='200072 验证密码为纯字符时，下一步按钮不可点击',
            result=case_result,
